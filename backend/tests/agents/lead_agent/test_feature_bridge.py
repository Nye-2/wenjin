"""Tests for live feature-bridge execution helpers and chat cards."""
...
from src.agents.lead_agent import feature_bridge_catalog
...
async def test_build_workspace_feature_overview_includes_skill_mapping(
    ...
    monkeypatch.setattr(feature_bridge_catalog, "get_db_session", lambda: _FakeSessionContext())
    monkeypatch.setattr(feature_bridge_catalog, "WorkspaceService", _FakeWorkspaceService)

    overview = await feature_bridge_catalog.build_workspace_feature_overview(
        "ws-1",
        user_id="user-1",
    )
