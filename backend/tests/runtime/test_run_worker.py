...
from src.runtime.runs import RunManager, RunStatus, run_thread_turn
...
async def test_run_thread_turn_swallows_cancelled_error_and_marks_interrupted():
    ...
    with patch("src.runtime.runs.worker.set_thread_status", new=AsyncMock()) as set_thread_status:
    ...
    assert latest.status == RunStatus.interrupted
...
async def test_run_thread_turn_skip_idle_status_when_other_run_is_active():
    ...
    assert latest_first.status == RunStatus.interrupted
    assert latest_second.status in (RunStatus.pending, RunStatus.running)
