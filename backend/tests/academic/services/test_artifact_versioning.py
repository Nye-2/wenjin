...
from src.dataservice_client.contracts.asset import WorkspaceArtifactPayload


def _projection(**overrides) -> WorkspaceArtifactPayload:
    now = datetime.now(UTC)
    return WorkspaceArtifactPayload(
...
@pytest.mark.asyncio
async def test_create_delegates_to_dataservice_workspace_artifact_command() -> None:
    dataservice = AsyncMock()
    service = ArtifactService(dataservice=dataservice)
    expected = _projection(id="artifact-new")
    dataservice.create_workspace_artifact.return_value = expected
    ...
    command = dataservice.create_workspace_artifact.await_args.args[0]
...
async def test_find_latest_and_list_versions_delegate_to_dataservice() -> None:
    ...
    dataservice.find_latest_workspace_artifact.return_value = latest
    dataservice.list_workspace_artifact_versions.return_value = versions
    ...
    dataservice.find_latest_workspace_artifact.assert_awaited_once_with(
        workspace_id="ws-1",
        artifact_type="research_idea",
        title="My Research",
    )
    dataservice.list_workspace_artifact_versions.assert_awaited_once_with(
        workspace_id="ws-1",
        artifact_type="research_idea",
        title="My Research",
    )
...
async def test_crud_and_lineage_delegate_to_dataservice() -> None:
    ...
    dataservice.get_workspace_artifact.return_value = artifact
    dataservice.list_workspace_artifacts.return_value = [artifact]
    dataservice.update_workspace_artifact.return_value = artifact
    dataservice.delete_workspace_artifact.return_value = True
    dataservice.get_workspace_artifact_lineage.return_value = lineage
    ...
    dataservice.list_workspace_artifacts.assert_any_await(
        workspace_id="ws-1",
        artifact_type="research_idea",
        status=None,
        limit=50,
    ...
    update_command = dataservice.update_workspace_artifact.await_args.args[1]
