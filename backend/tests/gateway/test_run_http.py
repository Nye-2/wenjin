...
async def test_get_run_or_404_enforces_optional_thread_scope() -> None:
    ...
    resolved = await get_run_or_404(manager, record.run_id)
    assert resolved.run_id == record.run_id

    with pytest.raises(HTTPException) as scoped_exc:
        await get_run_or_404(manager, record.run_id, thread_id="thread-2")
...
async def test_cancel_run_with_wait_returns_204_and_interrupts_task() -> None:
    ...
    response = await cancel_run_with_http_response(
        run_manager=manager,
        record=record,
        action="interrupt",
        wait=True,
...
async def test_cancel_run_raises_409_for_non_cancellable_run() -> None:
    ...
    with pytest.raises(HTTPException) as exc:
        await cancel_run_with_http_response(
            run_manager=manager,
            record=record,
            action="rollback",
            wait=False,
...
async def test_maybe_cancel_then_wait_returns_none_without_action() -> None:
    ...
    response = await maybe_cancel_then_wait(
        run_manager=manager,
        record=record,
        action=None,
        wait=True,
