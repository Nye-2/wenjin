...
from src.gateway.services.run_lifecycle import launch_thread_run, sse_consumer
from src.runtime.runs import DisconnectMode, RunManager, RunRecord, RunStatus
...
async def _launch_for_test(
    *,
    ...
    await launch_thread_run(
        ...
        metadata={},
        kwargs={},
        on_disconnect=DisconnectMode.cancel.value,
...
async def test_launch_thread_run_dispatches_to_celery_worker(monkeypatch):
    ...
    monkeypatch.setattr(task_module, "execute_run", dummy_task)
    monkeypatch.setattr(run_lifecycle_module.asyncio, "create_task", _fake_create_task)

    record = await launch_thread_run(
        handler=handler,  # type: ignore[arg-type]
        run_manager=manager,
        bridge=bridge,
        actor_id="user-1",
    ...
    assert dummy_task.calls[0]["queue"] == "long_running"
