...
CHECK_DESCRIPTIONS: Mapping[str, str] = {
    ...
    "failure_recovery_workflow": "Run task failure recovery, resume, and retry seed workflow gate tests.",
...
CHECK_FIX_HINTS: Mapping[str, str] = {
    ...
    "artifact_refresh_workflow": "Run `PYTHONPATH=. uv run pytest tests/task/test_workspace_feature_handler_matrix.py tests/task/test_store.py::TestTaskStorePostgres::test_mark_task_completed_publishes_canonical_task_activity tests/task/test_workspace_feature_frontend_sync.py -q` and fix artifact refresh regressions.",
    "artifact_followup_workflow": "Run `PYTHONPATH=. uv run pytest tests/services/test_artifact_followup_workflow_gate.py tests/agents/lead_agent/test_thread_feature_flow.py tests/services/test_workspace_activity_service.py::test_task_activity_promotes_result_artifact_as_retry_seed -q` and fix artifact follow-up route seed regressions.",
    "failure_recovery_workflow": "Run `PYTHONPATH=. uv run pytest tests/services/test_failure_recovery_workflow_gate.py tests/agents/lead_agent/test_thread_feature_flow.py tests/task/test_workspace_feature_frontend_sync.py -q` and fix failed task resume/retry regressions.",
