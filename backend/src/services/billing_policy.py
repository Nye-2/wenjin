...
def calculate_weighted_tokens(
    ...
    """Calculate weighted model tokens under either new or legacy policy fields."""
    policy = _policy_config(model_policy)
    usage = _normalize_usage_dict(token_usage)
    if _is_legacy_model_policy(policy):
...
def calculate_model_usage_credits(
    *,
    ...
    if _is_legacy_model_policy(policy):
        ...
        raw_cost_cny = _legacy_raw_cost_cny(policy, billable_usage)
        raw_cost_usd = (
...
def _is_legacy_model_policy(policy: dict[str, Any]) -> bool:
    return any(
...
def _legacy_raw_cost_cny(policy: dict[str, Any], usage: dict[str, int]) -> float:
    return (
