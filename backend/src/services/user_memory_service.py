...
KNOWLEDGE_EXTRACTION_PROMPT = """你是问津的长期学术记忆提取器。从以下对话中提取对未来协作稳定有价值的学术相关知识点。仅返回 JSON 数组:
[
  {{
...
- 优先提取跨会话仍有指导价值的信息，忽略一次性操作指令和临时任务状态
- 不提取密钥、凭证、隐私敏感标识或不应长期保存的信息
- 合并相近的知识点，避免重复
- 如信息只是当前 feature 执行参数且不代表长期偏好或研究状态，不要写入记忆
...
仅返回有效 JSON 数组，不要 Markdown 代码块、注释或其他内容。"""
