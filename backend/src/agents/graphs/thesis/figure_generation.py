...
# ---------------------------------------------------------------------------
_PLAN_FIGURE_PROMPT = """你是问津 Compute 的学术图表规划专家。根据以下信息，规划一张可执行、可落稿的学术图表。
...
约束:
- 只基于给定描述和章节上下文规划，不补造不存在的数据或实验结果
- 优先选择最简单且能准确表达论证关系的策略
...
# ---------------------------------------------------------------------------
_GENERATE_MERMAID_PROMPT = """你是问津 Compute 的 Mermaid 图表语法专家。根据以下规划生成可渲染 Mermaid 代码。
...
3. 布局清晰、层次分明
4. 不补造规划之外的模块、数据或结论
5. 仅返回 Mermaid 代码，不要包含 ```mermaid 标记"""

_GENERATE_PYTHON_PROMPT = """你是问津 Compute 的 Python 数据可视化专家。根据以下规划生成 matplotlib 绑定代码。
...
5. 如果没有真实数据，只生成占位示例并在图题或注释中标明示意
6. 仅返回完整的 Python 代码，不要包含 ```python 标记"""

_GENERATE_KLING_PROMPT = """你是问津 Compute 的 AI 图片生成提示词专家。根据以下规划生成学术插图提示词。
