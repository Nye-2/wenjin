...
# LLM Prompts
# ---------------------------------------------------------------------------
_REVIEW_PROMPT = """你是问津 Compute 的学位论文章节审稿人。请审阅以下论文章节，返回 JSON。
...
_REVISE_PROMPT = """你是问津 Compute 的学位论文章节修改专家。请根据审稿意见修改以下章节。
...
3. 保持学术写作风格
4. 确保引用格式一致
5. 不新增无法从上下文支持的引用、数据或实验结论；必要处标注待补充
