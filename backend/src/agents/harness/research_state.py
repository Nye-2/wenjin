...
class ResearchStateV1(BaseModel):
    ...
    workspace_map_summary: dict[str, Any] = Field(default_factory=dict)
    ...
    evidence_packet: list[dict[str, Any]] = Field(default_factory=list)
...
def compact_research_state(
    *,
    ...
    for report in expert_reports:
        ...
        claim_inventory.extend(_dict_items(_nested(report, "claim_inventory", "claims"), id_key="claim_id"))
        evidence_packet.extend(_dict_items(_nested(report, "evidence_packet", "items"), id_key="evidence_id"))
        ...
        unresolved_blockers.extend(
            _string_items(_nested(report, "evidence_packet", "gate_decision", "blocking_reasons"))
        )
        unresolved_blockers.extend(_string_items(_nested(report, "evidence_packet", "gate_decision", "warnings")))
        accumulated_next_actions.extend(_string_items(report.get("next_actions")))
    return ResearchStateV1(
        execution_id=execution_id,
        goal=goal,
        research_brief=research_brief,
        workspace_map_summary=workspace_map_summary or {},
        decisions=decisions or [],
        claims=_dedupe_by_key(claims, "claim_id"),
        claim_inventory=_dedupe_by_key(claim_inventory, "claim_id"),
        evidence_index=_dedupe_by_key(evidence_index, "evidence_id"),
        evidence_packet=_dedupe_by_key(evidence_packet, "evidence_id"),
