...
_JSON_SYSTEM_RULES = "\n".join(
    [
        ...
        "只使用提供的上下文；不要编造具体论文、专利号、实验指标、引用或事实。",
        "如果关键信息不足，请给出保守占位内容，并在文本中明确说明需要补充或核验。",
    ]
)
    ...
    *,
    ...
    lines.extend(
        [
            ...
            "2. 缺失列表返回空数组，缺失对象返回空对象，不要返回 null。",
            "3. 不要编造未提供的论文、专利号、实验结果、统计指标或引用事实。",
            "4. 如果缺关键事实，请在字段值中明确标注“待补充/待核验”。",
            "5. 先保证结构正确，再保证语言凝练、信息密度高、可直接使用。",
        ]
    )
