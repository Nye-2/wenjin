"""HTTP helpers for ChatTurnRun transport."""
...
from src.gateway.services.chat_turn_lifecycle import sse_consumer
from src.observability.prometheus import observe_run_wait
from src.runtime.chat_turns import ChatTurnRunManager, ChatTurnRunRecord, ChatTurnRunStatus
...
async def get_chat_turn_or_404(
...
async def await_chat_turn_task(
...
async def cancel_chat_turn_with_http_response(
    *,
    run_manager: ChatTurnRunManager,
    record: ChatTurnRunRecord,
    ...
    if not cancelled:
        raise HTTPException(
            status_code=409,
            detail=(f"Run {record.run_id} is not cancellable (status: {record.status.value})"),
        )

    if wait:
        await await_chat_turn_task(record, run_manager=run_manager)
        return Response(status_code=204)

    return Response(status_code=202)


async def maybe_cancel_chat_turn_then_wait(
    *,
    run_manager: ChatTurnRunManager,
    record: ChatTurnRunRecord,
