...
The existing `graph.py` already has `GraphTemplateRegistry` and `create_default_subagent_graph`.
Append the following two new functions to the file:

```python
# Append to src/subagents/graph.py (after existing code)
