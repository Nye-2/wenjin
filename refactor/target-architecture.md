...
问津当前已经形成较完整的 workspace feature 执行主链：
...
这条链路解决了早期 CrewAI/workflow 能力的产品化问题，但 chat 链路后来承担了过多职责：对话、意图识别、feature 触发、确认、stream、billing、memory capture、状态投影和部分 subagent 能力。结果是 chat 从用户入口侵入了 feature 执行域，引起架构漂移。
...
4. 不保留旧的 chat-driven `run_workspace_feature` 自由工具调用路径。
...
任何 router、chat、panel、agent tool 都不得直接调用 FeatureExecutionHandler、task handler 或 graph。
```
...
4. 当前 execution session 处于 `awaiting_user_input` 且用户回复补充信息 -> `feature_resume`
...
2. `run_workspace_feature` 不作为 pure chat lead-agent 的自由执行工具暴露。
