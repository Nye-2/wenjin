...
`sandbox_execution_summary` 会把 `execution_lifecycle` 的 bounded lifecycle count、job status、exit code 和 explicit `/workspace/tmp/tasks/.harness/outputs/**` output refs 带入后续 harness context。lifecycle 自身也只保留 explicit internal output refs，且 artifact count 只统计 layout 判定为用户可审阅的 `/workspace/outputs/**` 或 `/workspace/reports/**` 产物。context assembly 只在 `sandbox_execution_summary.output_refs` 中保留这类可通过 `sandbox.read_output_ref` 读取的 refs，其他 internal/protected/non-output paths 仍按普通规则过滤。
