...
14. `capability.v2.methodology` 是 capability 级研究方法合同，不是展示路由或第二套 graph。它可声明 `archetype`、阶段 `stages`、阶段产物、claim two-pass audit、retrieval escalation 和 completion research surfaces；TeamKernel 会把它作为 bounded `methodology_contract` 投影给成员。`claim_policy.mode=two_pass` 必须同时声明 `claim_inventory` 与 `claim_evidence_map` 这类抽取/核验证据产物，并绑定 `claim_evidence_alignment` surface。
15. 一线 SCI worker skill 必须在 output schema 中声明 `expert_report`，并在 Output Contract 中要求 `wenjin.expert_report.v1`。`expert_report` 负责把 bounded claims、evidence refs、artifact refs 和 warnings 交给 TeamKernel 汇总，不替代 `outputs[]`、Prism review item 或 room commit。
16. Prompt Pack v3 在 v2 的 `expert_report`、`claim_inventory`、`evidence_packet` 和可选 `research_brief_delta` 之上，要求每个 skill prompt 显式消费 `methodology_contract`：按 stage 对齐 `required_artifacts`，在 `claim_policy.mode=two_pass` 时先抽取 claim inventory 再核验证据，按 `retrieval_policy.escalation` 使用 Library、上游输出、artifact 和必要的外部检索。Skill prompt 必须要求无证据时输出 `insufficient_evidence`，不得伪造 DOI、BibTeX key、venue、year、metric 或 artifact ref；query planning、source scouting、screening、literature synthesis、citation audit、method design、evidence analysis、reproducibility audit、manuscript architecture/writing、review critic、proposal/patent/software-copyright worker 都必须带有角色特定的方法提示，而不是只依赖通用安全段落。
...
5. Lead Agent v2 初始 state 注入 `capability_policy`：`mission`、`context_policy`、`sandbox_policy`、`review_policy`、`citation_policy`、`research_evidence`、`methodology`、`quality_gates`。
...
1. 新 capability 先改 v2 seed/schema，再改 runtime/frontend/docs；研究型 capability 若需要固定方法路径，必须优先声明 `methodology`，不得把阶段、claim audit 和 retrieval escalation 只写进 worker prompt。
