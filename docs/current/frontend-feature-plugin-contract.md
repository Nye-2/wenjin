...
- 后端职责:
  ...
  - Chat Agent 根据 workspace mission catalog / skills 上下文判断是否调用 `launch_feature` tool；catalog preload 会把用户可见 capability 的 `routing` 合约压缩为 route-card prompt，不使用 embedding 路由或前端关键词硬路由
...
- route-card、trigger phrases、minimum_context 字段名、内部决策标签不得直接显示给用户。
