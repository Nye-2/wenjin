...
`TaskReport.review_packet(schema=wenjin.review_packet.v1)` 是 Academic Harness 的候选结果语义 envelope。RunView 读取 review packet 计算 reviewPacket、质量提示、sandbox/prism 计数和 `preview_results` action；LiveWorkflowPanel 通过 `buildWorkspaceResultPreviewsFromReviewPacket()` 把 packet items 投影为只读预览。保存行为不能直接消费 packet item id；真正写入仍必须走 `TaskReport.outputs`、canonical review items、room commit 或 Prism apply/reject/revert。
