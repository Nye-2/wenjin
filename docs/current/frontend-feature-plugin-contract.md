...
- `frontend/app/(workbench)/workspaces/[id]/components/live-workflow/evidence-ledger.ts`
...
- `useLiveWorkflowViewModel` 负责 selected run、outputs、previews、review items、default checked ids、high-risk output ids 和 sandbox count。
- `evidence-ledger.ts` 负责从 `ExecutionRecord` + staged previews 派生 Evidence Ledger rows，包括 outputs、node outputs、claim 审计、citation 审计和 sandbox trace。
- ReviewView / EvidenceView 只消费 projection，不重复解析 quality gates、claim/citation schema 或 node outputs。
