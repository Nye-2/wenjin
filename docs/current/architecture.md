...
| Live workflow presenter | `frontend/app/(workbench)/workspaces/[id]/components/live-workflow/useLiveWorkflowViewModel.ts` `frontend/app/(workbench)/workspaces/[id]/components/live-workflow/evidence-ledger.ts` | Current run / Review / Evidence Ledger 的本地投影、默认勾选和证据风险派生 |
...
45. Review 默认勾选、accept-all 可用性和 Evidence Ledger 风险展示必须来自 `useLiveWorkflowViewModel` / `evidence-ledger.ts`，不得在 ReviewView、EvidenceView 或 chat result card 中重复推导
