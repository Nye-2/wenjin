...
51. 专家实名制展示契约分两层：`src.contracts.team_presentation` 是共享纯数据契约，定义 DataService-backed agent template `expert_profile`、capability `extensions.team_presentation` 和 profile resolve 规则；`src.contracts.team_expert` 只定义运行态 `expert_snapshot` / `expert_preview_item` sanitizer。DataService catalog、seed/admin validation 和 TeamKernel policy 可以依赖 `team_presentation`，但 DataService/domain 代码不得 import TeamKernel 运行态 contract；runtime/services 也不得 import `src.dataservice.domains.*` 内部模块。
52. 专家运行中摘录是 best-effort UX projection：subagent 可以通过 `SubagentContext.emit_expert_snapshot()` 发布 bounded snapshot，TeamKernel 把它合并进 `ExecutionNodeRecord.node_metadata.harness.expert_snapshots`，但 snapshot 记录/事件发布失败不得让成员任务失败。最终用户可见团队事实仍来自 hydrated `ExecutionRecord.node_states`，前端只通过 `frontend/lib/execution-run-view.ts` 投影 `metadata.team === true` 的节点，不新增专家状态 store 或解析 raw tool JSON。
