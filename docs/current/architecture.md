...
4. `/workspace/outputs/harness/**` 只保存 bounded stdout/stderr、tool refs 和内部执行证据，不能作为用户面对的 artifact 注册或引用。
