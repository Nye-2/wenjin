...
- 架构守卫覆盖 auth DB dependency、runtime legacy artifact naming、Prism adapter public route prefix、Prism adapter DB session dependency、memory runtime Knowledge DataService boundary、dashboard runtime DataService boundary、workspace runtime DataService boundary、admin catalog runtime DataService boundary、reference library runtime DataService boundary
