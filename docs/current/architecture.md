...
48. Research-task eval 是 deterministic harness 质量基础设施，不调用模型、不创建第二套运行态。`backend/src/agents/harness/research_task_eval.py` 只审计 `TaskReport`、review items 和 hydrated node metadata 是否覆盖文献证据、sandbox 实验复现和写作审阅 surface；它用于 release gate / mock E2E，不替代 TeamKernel、quality gates 或人工 review。
