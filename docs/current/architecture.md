...
48. Research-task eval 是 deterministic harness 质量基础设施，不调用模型、不创建第二套运行态。`backend/src/agents/harness/research_task_eval.py` 只审计 `TaskReport`、review items、hydrated node metadata 和 TeamKernel `runtime_state.quality_gates` 是否覆盖文献证据、sandbox 实验复现、写作审阅、`workflow_trace` 成员执行痕迹 surface，以及可选 `citation_strength` / `experiment_interpretation` / `paper_relevance` / `statistical_robustness` / `writing_semantic_preservation` / `writing_academic_style` outcome-quality surfaces；surface 枚举、默认 surface 和 capability policy 解析统一在 `backend/src/agents/harness/research_eval_surfaces.py`，避免 evaluator 主文件继续吸收配置解析逻辑。它用于 release gate / mock E2E，也被 TeamKernel quality gates 复用来检查 capability 声明的 runtime 可验证 surfaces，但不替代人工 review 或创建新的执行 runner。
