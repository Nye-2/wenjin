...
`sandbox_execution_summary` 还会吸收 `execution_lifecycle(schema=wenjin.sandbox.execution_lifecycle.v1)` 的 bounded 字段：lifecycle count、job statuses、exit codes，以及明确可通过 `sandbox.read_output_ref` 回看的 `/workspace/tmp/tasks/.harness/outputs/**` output refs。context assembly 只在这个 summary 字段内保留这些 explicit output refs，其他 protected/internal/non-output paths 继续被过滤。
