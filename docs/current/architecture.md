...
| Capability Domain | `backend/seed/capabilities/` `backend/src/dataservice/domains/catalog/` `backend/src/services/capability_loader.py` | `capability.v2` mission schema、DataService Catalog SSOT、methodology contract、graph_template、policy preload |
...
| Agent Runtime | `backend/src/agents/lead_agent/` | graph compile、subagent orchestration、runtime context assembly、TaskReport |
...
| Live workflow presenter | `frontend/app/(workbench)/workspaces/[id]/components/live-workflow/useLiveWorkflowViewModel.ts` `frontend/lib/execution-run-view.ts` | Current run / Review / Evidence Ledger 的统一投影、只读证据摘要和证据风险派生 |
...
11. Workspace memory runtime 只通过 WorkspaceMemory DataService client 读写一份 workspace 绑定的隐藏 Markdown 文档；旧 cross-workspace `user_knowledge`、`memory_facts`、memory compaction 和默认 Celery memory capture 不再作为运行态路径存在。
...
43. Execution node detail 的运行态事实源是 `ExecutionNodeRecord`；`ExecutionRecord.node_states` 不得作为 gateway/API 的节点详情事实源。`execution_nodes` 必须以 `(execution_id, node_id)` 作为数据库唯一边界，DataService 节点写入必须走 repository-level upsert，不得恢复先查后写或让 `node_states_json` 成为第二套写入事实源。Gateway/API 可以在 read path 由 `ExecutionService` 从 `execution_nodes` hydrate `node_states` 给前端展示，但不能反向把 `node_states` 当成运行态写入或节点详情来源
44. DataService client 的 execution / generation API 必须通过领域 mixin 承载，避免 `AsyncDataServiceClient` 主文件继续膨胀成跨领域热点
45. Agent harness 只能由 Lead Agent graph / TeamKernel subagent 调用；不得由 Chat Agent、router、前端或新 execution stream 直接调用；tool calls、diff、事件和产物必须投影回现有 `ExecutionNodeRecord`、DataService execution events、sandbox job/artifact 和 review/result-card 流。`library_read` / `prism_file_read` / `workspace_memory_read` / `prism_read` / `citation_parser` / `artifact_create` 是 subagent business-context tools，只能读取 bounded workspace snapshot 或返回 staged payload，不能直接提交 rooms、写 Prism 或物化 artifact。
46. Codex 和 deer-flow 只能作为 harness 模式参考：可借鉴 argv-first command contract、output budget、diff tracking、protected paths、tool error recovery、dangling tool-call repair、loop guard 和 sandbox provider hygiene；不得引入 Codex SDK/app-server、deer-flow agent factory、cc-switch bridge、第二套 thread/run model、第二套 execution stream 或通用 bash runtime 作为当前 Wenjin 执行事实源。外部项目取舍不再单独维护过程矩阵；当前约束以本文件和 release gate 为准。
47. Sandbox artifact review 必须携带可复现元数据：`source_script`、`dataset_paths`、`source_task_id`、`sandbox_environment_id` 和 `content_hash` 从 harness discovery / artifact manifest 进入 workspace asset metadata、sandbox artifact `reproducibility_json`、DataService review item 和 `TaskReport.review_items[*].reproducibility`。不得只以 path/hash 展示产物而丢失复现实验链路。
48. Research-task eval 是 deterministic harness 质量基础设施，不调用模型、不创建第二套运行态。`backend/src/agents/harness/research_task_eval.py` 只审计 `TaskReport`、review items、hydrated node metadata、`TaskReport.review_packet` 和 TeamKernel `runtime_state.quality_gates` 是否覆盖文献证据、sandbox 实验复现、写作审阅、`workflow_trace` 成员执行痕迹 surface，以及可选 `citation_strength` / `experiment_interpretation` / `paper_relevance` / `statistical_robustness` / `writing_semantic_preservation` / `writing_academic_style` / `review_packet_completeness` / `claim_evidence_alignment` outcome-quality surfaces；surface 枚举、默认 surface 和 capability policy 解析统一在 `backend/src/agents/harness/research_eval_surfaces.py`，避免 evaluator 主文件继续吸收配置解析逻辑。它用于 release gate / mock E2E，也被 TeamKernel quality gates 复用来检查 capability 声明的 runtime 可验证 surfaces，但不替代人工 review 或创建新的执行 runner。
    TeamKernel 的 `research_evidence_required` gate 只评估可从当前 `ExecutionNodeRecord.node_metadata.harness` 证明的 surfaces（`workflow_trace`、`output_ref_reuse`、`citation_strength`、`experiment_interpretation`、`paper_relevance`、`statistical_robustness`），失败时进入 `revise_existing`；依赖最终 `TaskReport.outputs` / `review_items` 的 literature、experiment、writing、Prism semantic preservation 和 Prism academic style 仍保留在 final report / release gate eval 中，避免在 review item staging 前误卡运行。
    citation/source audit evidence 必须来自 bounded `citation_source_audit` findings，明显 fabricated / missing / unsupported 或 high/critical/blocking severity 的 refs 不得作为文献证据通过；`citation_strength` 比文献结构覆盖更严格，只接受 supported/verified/low-risk 等强证据，weak-only refs 会失败。`paper_relevance` 读取现有 `node_metadata.harness.paper_relevance_summary`，要求至少一个带 `source_id` 或 `citation_key` 的 topic-aligned ref，且不能存在 off-topic ref；它不运行模型 judge、不自行检索、不把弱相关文献当作贴题证据。
    `experiment_interpretation` 读取现有 `node_metadata.harness.experiment_interpretation_summary` 和 `reproducibility_summary`，要求方法摘要、指标、已验证结果、限制、artifact 和 dataset path 同时存在并与可复现实验路径对齐；它不读取 raw stdout/stderr、不运行模型 judge、不新增运行态。`statistical_robustness` 读取现有 `node_metadata.harness.statistical_robustness_summary` 和 `reproducibility_summary`，要求统计方法、样本量、指标、至少一项通过的稳健性检查、限制、artifact 和 dataset path 同时存在，且 artifact/dataset path 与可复现实验路径对齐；critical/high/blocking 级 failed robustness check 不得通过。
    `writing_semantic_preservation` 读取 canonical Prism review item 的 `preview.content_contract` 和 `preview.semantic_contract`，要求每个 Prism 改稿都有低风险结构语义保护证据，不能出现 high-risk、结构无效、claim/citation/equation/table preservation 失败；它只做 bounded structural heuristic / upstream contract 审计，不读取全文原稿、不运行模型 judge、不绕过 review-first flow。
    `writing_academic_style` 读取 canonical Prism review item 的 `preview.academic_style_contract`，要求每个 Prism 改稿都有低风险学术风格信号、分数不低于门槛且没有 AI-meta/口语化等反模式；若上游合同提供 `style_delta(schema=wenjin.prism.academic_style_delta.v1)`，eval 会强制要求 pending score 相对 baseline score 正向提升，防止深度改稿比原稿退步。该合同只能通过 `PrismFileChangeUpsertPayload.academic_style_contract` 从 writer output mapping 进入 canonical review item；Prism academic style schema、入库 allowlist bounding、projection delta 重算和 eval delta validation 统一由 `backend/src/services/prism_review_contracts.py` 提供，DataService 入库时先剔除 raw before/after 等非合同字段，projection 再净化信号、count 和 delta，重算 `score_delta/improves_academic_style`，不暴露 pending content、raw diff、改前原文或全文原稿，也不替代人工编辑判断。
    `workflow_trace` 只读取 `node_metadata.harness.member_execution_transcript`，要求至少存在已完成工具活动，并汇总工具、变更、sandbox ids、scratch refs、usage、billing 和 duration；它不读取 raw tool JSON、不重算扣费、不新增运行态。TeamKernel 与普通 Lead runtime 都必须把 `prism_file_change` output declaration stage 成 canonical Prism review item，不能把写作 evidence 留在成员 raw output 或普通 room output 中；`.tex` review item 还必须在 `preview.content_contract` 中提供 bounded 结构摘要，`main.tex/project:main` 必须是完整 LaTeX document，并在 `preview.semantic_contract(schema=wenjin.prism.semantic_contract.v1)` 与 `preview.academic_style_contract(schema=wenjin.prism.academic_style_contract.v1)` 中提供 bounded 质量摘要；旧 `prism.file_changes[]` surface 不承载这些质量合同。
49. Capability execution worker ownership 属于 DataService execution aggregate：worker 必须先通过 `claim_execution_lease` 写入 `ExecutionRecord.runtime_state.execution_lease` 和 `worker_task_id`，运行期间通过 `heartbeat_execution_lease` 延长 lease；丢失 lease 的 worker 必须停止执行。Startup reconcile 只能回收缺失 lease 或 lease 已过期的 in-flight execution，并在终态化时清理 stale lease runtime state。不得新增 Redis-only worker lock、第二套 execution ownership 表，或让 Gateway/Task 绕过 DataService 直接判定 execution owner。
49. TeamKernel core phase 必须支持上下文交接：每个 core batch 完成后要把当前 batch 的 `node_metadata.harness` 同步回同一 `workspace_data`，让后续 batch 的成员能看到最新 `reproducibility_summary`、`experiment_interpretation_summary`、`member_execution_transcript` 和安全 `scratch_refs`。需要严格流水线的 capability 应把 `team_policy.limits.max_parallel_invocations` 设为 1；可并发成员仍可通过更大的 parallel limit 同批运行，但同批成员不得假设能读取彼此结果。`sandbox.read_output_ref` 是 `sandbox.read_file` 与 `sandbox.run_python` 的安全 companion：只能读取 explicit internal output ref，不能让 internal harness refs 被 list/search/artifact discovery 暴露。
50. TeamKernel 成员上下文必须把 capability policy 的 research evidence contract 投影为 `research_evidence_requirements(schema=wenjin.team.research_evidence_requirements.v1)`，包含 `quality_gate=research_evidence_required`、ordered required surfaces、runtime-enforced surfaces 和 bounded guidance；同时把 capability methodology 投影为 `methodology_contract(schema=wenjin.team.methodology_contract.v1)`，只包含 archetype、阶段、必备产物、claim audit、retrieval escalation 和 completion surfaces，不携带 routing、raw graph 或 display 文案。Harness context budget policy 统一在 `backend/src/agents/harness/context_budget_policy.py`，`context_assembly.py` 只负责构造 bounded bundle；budget policy 必须按 required runtime surface 保护对应 evidence summary：`workflow_trace` 保护成员执行痕迹和安全 scratch refs，`output_ref_reuse` 保护 sandbox execution summary / output-ref recovery / member transcript，`experiment_interpretation` 保护 experiment interpretation 与 reproducibility summary，`statistical_robustness` 保护 robustness 与 reproducibility summary；只有在非保护上下文和结构性 sandbox/workspace 描述已压缩后，才允许把这些 required evidence summary 降级为空。
51. Lead Agent workspace context assembly 统一由 `backend/src/agents/lead_agent/v2/runtime_context.py::RuntimeContextAssembler` 负责。`LeadAgentRuntime` 只调用 assembler 并把结果分发给 subagent，不再直接实现 source page projection、dataset provenance、workspace history、sandbox context 或 Prism pending review summary 加载；普通 runtime 与 TeamKernel 必须复用同一 assembler 边界，避免两套路由上下文策略。
52. 专家实名制展示契约分两层：`src.contracts.team_presentation` 是共享纯数据契约，定义 DataService-backed agent template `expert_profile`、capability `extensions.team_presentation` 和 profile resolve 规则；`src.contracts.team_expert` 只定义运行态 `expert_snapshot` / `expert_preview_item` sanitizer。DataService catalog、seed/admin validation 和 TeamKernel policy 可以依赖 `team_presentation`，但 DataService/domain 代码不得 import TeamKernel 运行态 contract；runtime/services 也不得 import `src.dataservice.domains.*` 内部模块。
53. 专家运行中摘录是 best-effort UX projection：subagent 可以通过 `SubagentContext.emit_expert_snapshot()` 发布 bounded snapshot，TeamKernel 把它合并进 `ExecutionNodeRecord.node_metadata.harness.expert_snapshots`，但 snapshot 记录/事件发布失败不得让成员任务失败。最终用户可见团队事实仍来自 hydrated `ExecutionRecord.node_states`，前端只通过 `frontend/lib/execution-run-view.ts` 投影 `metadata.team === true` 的节点，不新增专家状态 store 或解析 raw tool JSON。
54. Chat Agent capability routing 是 LLM-only 渐进承诺：输入事实来自 DataService Catalog 的 `capability.v2.routing` route-card，输出只能是直接回答、单句澄清、二选一引导或调用 `launch_feature`。不得新增 embedding/vector index、关键词硬路由、第二套 router service、前端本地 capability matcher 或绕过 Chat Agent 的自动启动链路。
55. Capability prompt governance 是 Catalog validation，不是运行时 prompt indirection。`capability_skill.v2.worker.role_prompt` 保持 worker 唯一 prompt body；Prompt Contract v1、routing-depth 和 agent template public-safety 只在 seed/admin/DataService Catalog 写入与加载校验中执行，不新增第二套 prompt renderer、prompt manager 或 runtime routing layer。
56. Academic Harness v1 的结构化成果链路由 `expert_report(schema=wenjin.expert_report.v1)`、`TaskReport.review_packet(schema=wenjin.review_packet.v1)` 和 `ResearchStateV1` 组成。Subagent skill 通过 `expert_report` 输出 bounded claims / evidence refs / artifact refs / warnings；TeamKernel 将专家报告汇总成 review packet 和 compact research state，并把 research state 传给后续动态招募成员。`review_packet` 是语义预览与质量闭环 envelope，不是第二套 room commit 通道；保存仍走 `TaskReport.outputs`、DataService-backed `review_items`、ExecutionCommitService 或 Prism apply/reject/revert。前端只能通过 `frontend/lib/execution-run-view.ts` 与 result preview 投影层展示 review packet，默认不得把 raw expert report、schema id 或内部 harness refs 暴露给用户。
57. Academic Harness v2 在 v1 envelope 内增加四个结构化学术事实源：`research_brief.v1`、`claim_inventory.v1`、`evidence_packet.v1` 和 `academic_workspace_map.v1`。TeamKernel run start 从已有 bounded `workspace_data` 生成 academic workspace map summary，再构建 research brief；初始成员通过 member context 接收 brief/map，后续成员通过 enriched `ResearchStateV1` 接收 compact claim/evidence/artifact/blocker handoff。Prompt Pack v3 要求 skills 在 v2 的 `expert_report.claim_inventory` / `expert_report.evidence_packet` 之上消费 `methodology_contract`：按 stage 和 `required_artifacts` 对齐输出，按 two-pass claim audit 先抽取再核验，按 retrieval escalation 使用 Library、上游输出、artifact 和必要外部检索；缺证据必须显式标记 `insufficient_evidence` 或 warning/blocker。`claim_evidence_alignment` eval 会把 high/critical claim warning 视为失败。Capability methodology 是这一层的 catalog-side workflow contract，用于固定 Intent/Triage/Deepen/Synthesize、paper-build spine、two-pass claim audit 和 retrieval escalation 等方法路径；它不新增 runner、第二 event stream、embedding router、权限 profile 或 review packet commit path。
58. ChangeUnit commit recovery 只允许使用临时 `execution.result.change_unit_materialization` 进度 payload：每个已完成 unit 记录实际 room targets，失败态只折叠出 completed materializations，重试跳过 completed units，成功 finalize 必须删除该临时 key。不得把变更历史持久化成长期表、不得恢复 `accepted_ids` bridge，也不得让 ResultCard、review packet 或 room service 成为第二套 commit 事实源。Decision/Task 直接写入使用 `execution:{execution_id}:unit:{change_unit_id}` provenance key 进行 DataService room replay；普通用户更新仍保持原有 set/create 语义。

### 2.5 Workspace Sandbox Filesystem

每个 workspace 最多拥有一个 active sandbox environment。sandbox 文件系统使用统一 `/workspace` 虚拟根，workspace 类型只改变 profile guidance，不改变目录拓扑。当前 canonical layout 由 `backend/src/sandbox/workspace_layout.py` 生成并写入 `/workspace/.wenjin/manifest.json`：

| Virtual path | 用途 | 用户可见性 |
|---|---|---|
| `/workspace/main` | 主项目文件：论文主稿、proposal、专利草稿、软著材料等 | workspace surface |
| `/workspace/datasets` | 可复用输入数据、上传后可实验复用的数据材料；provenance 写入 `/workspace/datasets/manifest.json` | workspace surface |
| `/workspace/scripts` | 可复现实验脚本，尤其是 `source_script` 引用的 Python 脚本 | workspace surface |
| `/workspace/outputs` | 图表、指标、表格、机器可读结果等 reviewable artifacts | artifact review |
| `/workspace/reports` | 文献综述、实验报告、审稿/改稿计划等用户可读报告；artifact manifest 写入 `/workspace/reports/artifacts.json` | artifact review |
| `/workspace/tmp` | 任务临时草稿和 scratch 数据；任务级 scratch root 固定为 `/workspace/tmp/tasks` | hidden |
| `/workspace/.wenjin/env` | sandbox 内托管运行环境 | hidden/protected |
| `/workspace/.wenjin/cache` | sandbox 内托管缓存 | hidden/protected |

文件系统规则：

1. Agent tool calls 只能使用 `/workspace` virtual path，不暴露宿主机路径。
2. DataService Reference Library 是引用事实源；Library 不在 sandbox 内另建第二套事实源。需要实验复用的输入材料可以通过 source asset / dataset provenance 物化到 `/workspace/datasets`。
3. 用户可审阅产物只能注册 `/workspace/outputs/**` 或 `/workspace/reports/**`，但必须排除 guidance files、protected paths、internal harness refs。
4. `/workspace/tmp/tasks/.harness/**` 是 task scratch 下的 harness internal area；其中 `/workspace/tmp/tasks/.harness/outputs/**` 只保存 bounded stdout/stderr、tool refs 和内部执行证据，不能作为用户面对的 artifact 注册或引用。
5. 可复现实验产物必须同时具备 `/workspace/scripts/*.py` source script、`/workspace/datasets/**` dataset path、reviewable artifact path 和 content hash。
6. Prism 写作结果不直接覆盖 `/workspace/main` 或 Prism project；必须 stage 为 canonical `prism_file_change` review item，再由用户 preview/apply/reject。普通 Lead runtime 与 TeamKernel 共享 `backend/src/agents/lead_agent/v2/prism_review_staging.py` 生成 `PrismFileChangeUpsertPayload`，避免两套 mapping/hash/normalization 行为分叉。
7. Workspace-specific profile 只能给 `primary_files`、`script_paths`、`output_paths`、`report_paths` 提供建议路径；`validate_workspace_type_profile()` 防止 profile 指向 protected/internal 路径。
8. 任务级临时材料统一放在 `/workspace/tmp/tasks`，该路径通过 `task_scratch_root` 暴露给 agent context，但仍按 hidden scratch 处理，不进入 artifact discovery 或 review item。
9. `build_workspace_task_contract()` 是 task-scoped sandbox 路径的唯一完整来源，包含 scratch path、internal output-ref root、manifest paths 和 reviewable artifact roots；`build_agent_workspace_task_contract()` 是唯一 model-facing 安全投影，供 harness context 和 Lead-owned sandbox job metadata 复用，并且不暴露 raw `output_ref_root`。
10. `workspace_harness_output_ref_path()` 是内部 output ref 持久化路径的唯一生成入口；`output_budget.py`、sandbox lifecycle 和 context recovery 只能消费这个 contract 生成的 `/workspace/tmp/tasks/.harness/outputs/**` refs，不得各自实现 path sanitization。
...
- Chat Agent 从 Catalog preload 的 bounded route-card 判断用户体验路径：小问题直接回答，缺最小上下文则追问一句，歧义时给两个自然语言选择，明确多步产出才 launch
...
- `launch_feature` 必须按已持久化 user message id 派生的 `launch_idempotency_key` 幂等：agent retry 可以重复调用 tool，但同一 key 的重复调用只能返回原 execution，不得创建第二个 execution，也不得误报无关 lead-busy
...
- Lead Agent 启动后、graph compile / team iteration / workspace context load 之前必须执行 capability preflight：静态 graph task 必须有可解析 `skill_id` 或显式 `allow_skillless=true`；TeamKernel 的 `capability_skills` / overlay skills / template effective skills 必须存在于 Catalog；`required_tools` / `allowed_tools` / team `capability_tools` 必须解析到已知 callable team/harness tool；`research_evidence.required_surfaces` 和 `surface_enforcement` 必须是已知枚举。
- Preflight 失败返回配置错误 TaskReport，不得继续进入 subagent dispatch、compiler fallback、workspace mutation 或模型/tool credit 消耗；专用内置 subagent 的 skillless 例外必须写在 capability seed / DB capability 配置中，不能由 runtime 静默放行。
...
- 普通 runtime 与 TeamKernel 加载 workspace context 时都必须通过 `RuntimeContextAssembler`，并显式传入 `capability_policy`、`TaskBrief.context_requirements` 和 `user_id`；citation/library allowlists 不能因为 policy 丢失而退化为空
- Capability / skill / tool / research-surface 配置错误属于 launch-time preflight failure，不属于 member failure；TeamKernel 只有在 preflight 通过后才加载 workspace context、构建 academic workspace map、dispatch 成员。
- Static graph React 节点的工具白名单必须由 harness policy resolver 统一生成：task `tools` / `required_tools`、capability `runtime.allowed_tools` / `tool_policy.allowed_tools` / sandbox policy、skill `allowed_tools` / `sandbox_access` 只作为输入；最终 `ctx.tools` 以 `resolve_harness_policy` 结果为准。`sandbox_policy.mode=required` 时必须提供 bounded read/list/glob/grep/read_output_ref 以及 policy 允许的执行工具；被权限或 isolation 拒绝的 `required_tools` 必须记录为节点错误，不能退化为无工具 LLM 调用。
- Static graph 与 TeamKernel subagent 调用都必须有节点级 timeout。运行中的 static graph 节点必须和 Redis abort signal 竞速；收到 abort 后取消子任务、记录 `execution_nodes.status=cancelled`，再让 `ExecutionAborted` 冒泡到 runtime 生成 cancelled TaskReport。lead-busy 只由 execution terminal status 释放，不得由前端局部状态推断。
...
- Review 默认只自动勾选低风险 `ResultOutput`；当 runtime quality gates、`claim_evidence_map`、`citation_key_audit`、`unsupported_claims`、`fabrication_risks`、`missing_sources` 或 schema `contract_error` 暴露证据/引用/claim 高风险时，前端取消默认勾选并禁用一键“全部接受”
- Evidence Ledger 是 Review 的并列投影面，负责把候选 outputs、节点输出、claim 审计、citation 审计和 sandbox trace 放在同一可过滤列表中；它不替代后端质量门槛，也不直接写入 rooms
...
默认 workflow UX（Chat launch receipt、LiveWorkflowPanel、Runs drawer、ResultCard/CompletedView）不得展示 raw node input、raw node output、raw stdout/stderr、raw tool args 或 internal harness refs；诊断层也只能展示经过 bounded sanitization 的摘要、basename、计数、风险和下一步。
...
- skill seed 的 `role_prompt` 必须是可执行 instruction pack，并满足 Prompt Contract v1。`capability_skill.v2.worker.role_prompt` 仍是单一运行时 prompt body；schema/admin/seed lint 只校验 catalog 质量，不生成第二份 prompt、不接管运行时渲染。
- visible capability 的 `routing` route-card 必须提供足够深的正反例与 required `minimum_context` 说明，供 Chat Agent 做 LLM-only 渐进承诺。
- agent template 的 `expert_profile` / `persona_prompt` public-safety 校验属于 DataService Catalog seed/admin validation，避免公开画像暴露 internal ids、tools、logs、stdout/stderr 或 harness 术语。
- `LeadAgentRuntime` 将 `mission`、`context_policy`、`sandbox_policy`、`review_policy`、`citation_policy`、`research_evidence`、`methodology`、`quality_gates` 注入 `capability_policy`
...
- API Key 在 DataService 内用 `MODEL_SECRET_KEY` / `MODEL_SECRET_KEY_FILE` 加密；敏感 default headers 同样只允许 admin write/rotate，admin list/edit projection 只返回 `api_key_redacted` 和 redacted header，编辑时空 API Key 表示不替换原密钥。
- Runtime model cache 位于 `backend/src/services/model_catalog_cache.py`。Gateway lifespan 会 best-effort warmup；worker bootstrap best-effort warmup；`execute_run` 和 `execute_execution` 在每次任务进入 LLM runtime 前强制从 DataService refresh。cache projection 必须保留 `pricing_policy_id`；DataService 写入层要求 enabled 模型绑定 enabled `model_usage` policy。首次 bootstrap 会先 seed 默认 pricing policies，再把 env model seed 显式绑定到 `default-model-usage`；运行时 CreditService 只按模型绑定 policy 或模型名精确 policy 计费，缺失时最多使用保守 token policy，不得借用第一个可用 `model_usage` policy。
...
- Pricing policy 由 DataService `pricing_policies` 持久化，当前 policy kinds 包括 `global_credit`、`model_usage`、`capability`、`tool`、`sandbox`。`backend/src/dataservice/domains/pricing/seed_loader.py` 是默认定价事实源，创建 `default-global-credit`、`default-model-usage`、`default-capability`、`default-tool`、`default-sandbox`。Pydantic config schema 使用 `extra="forbid"`，避免旧字段静默进入计费。
...
Workspace memory 是 workspace 绑定的一份隐藏 Markdown 文档，供后台 prompt/context 和显式低频更新使用，不作为默认用户导航入口。
...
- `/workspace/tmp`：临时 scratch，不默认作为用户可审阅产物；`/workspace/tmp/tasks` 是 task-scoped scratch root
...
所有 workspace type 共用这套目录，不为 `sci`、`thesis`、`proposal`、`software_copyright`、`math_modeling` 或 `patent` 分叉 provider layout。sandbox 目录是闭合集，不把 DataService 的 Library / Prism files / Decisions / hidden workspace memory 复制成 `/workspace/library`、`/workspace/documents` 或其他平行事实源；需要进入实验的输入必须通过 source asset / dataset provenance 物化到 `/workspace/datasets`。垂直差异通过 `workspace_profile(schema=wenjin.workspace_sandbox.type_profile.v1)` 进入 `.wenjin/manifest.json`、DataService sandbox environment metadata 与 harness context：profile 只声明推荐 `primary_files`、`script_paths`、`output_paths`、`report_paths` 和简短规则，例如 `sci` 推荐 `/workspace/main/main.tex`、`/workspace/main/refs.bib`、`/workspace/scripts/analysis.py`、`/workspace/outputs/figures`、`/workspace/reports/experiment-report.md`。`WORKSPACE_SUPPORTED_TYPES` 和 `validate_workspace_type_profile()` 会在测试中确保所有 workspace type profile 仍使用这套通用 layout 根目录，不落入 protected/internal 路径。未知 workspace type 使用 `generic` profile。Lead-owned sandbox runtime 在 acquire 后会用 Lead 已知的 workspace type 刷新 mounted `/workspace/.wenjin/manifest.json`，并在 `get_or_create_environment` 时写入轻量 `workspace_layout` / `workspace_profile` metadata；已有 active environment 会合并新的 runtime metadata，避免 provider 初始 generic manifest、DataService 管理面与 subagent context profile 分裂。新增类型应优先扩展 profile，不应新增平行目录或 provider 分支。

初始化会写入 `/workspace/main/README.md`、`/workspace/datasets/README.md`、`/workspace/outputs/README.md`、`/workspace/reports/README.md`、空的 `/workspace/datasets/manifest.json(schema=wenjin.workspace_sandbox.dataset_provenance.v1)` 和空的 `/workspace/reports/artifacts.json(schema=wenjin.workspace_sandbox.artifact_manifest.v1)`；已有 README、dataset manifest 或 artifact manifest 不会被覆盖。layout guidance 路径由 `is_workspace_guidance_path()` 统一识别，包含这些 README、manifest 和 `.gitkeep`，在 `classify_workspace_path()` 中归为 `hidden`，不能被注册为用户可审阅 artifact。dataset manifest 是 agent/用户可维护的数据来源清单，用于记录 `/workspace/datasets/**` 下可复用输入材料的 `source_id`、`content_hash`、license 和 preparation notes；secrets、API keys、credentials 和 private tokens 不得写入其中。Lead-owned `sandbox.run_python` 会在同一个 workspace lease 内、脚本执行前把 bounded `workspace_file_summary.dataset_provenance` 安全合并进 `/workspace/datasets/manifest.json`：只接受 `/workspace/datasets/**` 数据文件，拒绝 manifest/README/.gitkeep、protected/internal/non-workspace refs，并按 path 保留用户已有条目优先权。tool-using agent 若被 capability/skill 同时授予 `sandbox.register_dataset`、`filesystem.write` 和 `filesystem.diff`，也可以通过这个专用工具追加一条安全 dataset provenance；该工具只写 `/workspace/datasets/manifest.json`，复用同一套 sanitization 和用户已有条目优先规则，并返回普通 harness `file_change`，不允许把 host path、secret refs 或 `/workspace/tmp/tasks/.harness/outputs/**` 注册成数据集。`/workspace/reports/artifacts.json` 是 agent/用户可维护的产物元数据清单，用于给 `/workspace/outputs/**` 和 `/workspace/reports/**` 的用户可审阅产物补充 title、description、artifact_kind、source_script、dataset_paths 和 notes；其中 `source_script` 只接受 `/workspace/scripts/**` 下的安全 `.py` 脚本路径，不接受主稿、输出文件、notebook/notes、guidance、internal 或 host path。`sandbox.register_artifact` 是唯一 model-facing artifact metadata 写工具，同样要求 `filesystem.write` 与 `filesystem.diff`，会拒绝 non-artifact、protected/internal、guidance 和 host path refs，并返回 manifest `file_change`。

受保护路径由同一 layout 常量下发给 harness policy：`.git/**`、`.env`、`*.pem`、`*.key`、`.wenjin/env/**`、`.wenjin/cache/**`、`.wenjin/manifest.json`。`HarnessPolicy` 默认值也绑定这组 layout protected paths，直接构造 policy 的 mock、测试或未来工具入口不得得到更宽的文件系统可见性。`build_agent_workspace_contract()` 还暴露 `operation_policy(schema=wenjin.workspace_sandbox.operation_policy.v1)`：direct write tools 是 `sandbox.write_file`、`sandbox.str_replace`、`sandbox.apply_patch`，允许根是 `main/datasets/scripts/outputs/reports/tmp`，并额外允许 `/workspace/*` 这类根目录直接项目文件以兼容既有主稿，拒绝 path class 为 protected/internal/guidance 的目标和任意新顶层目录树；manifest update tools 则固定为 `sandbox.register_dataset` 写 `/workspace/datasets/manifest.json`、`sandbox.register_artifact` 写 `/workspace/reports/artifacts.json`。`/workspace/tmp/tasks/.harness/**` 统一分类为 internal，不可注册为用户产物；其中 `/workspace/tmp/tasks/.harness/outputs/**` 直接 output ref 优先由 `sandbox.read_output_ref` 以 bounded preview 只读回看，便于 agent 继续检查被省略的 stdout/stderr、大文件或 diff 中段，但不能写入、替换、注册 artifact、列目录、glob 或 grep。layout guidance/manifest 路径（README、`.gitkeep`、`datasets/manifest.json`、`reports/artifacts.json`）不属于普通 agent-editable 文件；`sandbox.write_file`、`sandbox.str_replace` 和 `sandbox.apply_patch` 会拒绝直接修改这些路径，dataset/artifact manifest 只能通过 `sandbox.register_dataset` / `sandbox.register_artifact` 结构化维护。只有 `is_user_reviewable_workspace_artifact_path()` 判定通过的 `/workspace/outputs/**` 和 `/workspace/reports/**` 文件才可进入 sandbox artifact review；若 provider 能反解 resolved target，artifact discovery 还要求 symlink 或真实目标同样通过该 reviewable artifact predicate，因此指向 `.env`、`.wenjin/**` 或 `/workspace/tmp/tasks/.harness/**` 的链接即使位于 artifact roots 下也不得 staging。`sandbox.list_dir`、`sandbox.glob` 和 `sandbox.grep` 也必须过滤 protected/internal 路径，不只是 write tools 拦截直接访问；这些工具还必须按 resolved physical target 跳过指向 workspace 外部、protected target 或 internal target 的 symlink，不能把外部文件、受保护文件、内部 refs 或 physical host path 投影给 agent。`sandbox.read_file` 会直接拒绝 protected 路径和非 output-ref internal 路径；`sandbox.write_file`、`sandbox.str_replace` 和 `sandbox.apply_patch` 会直接拒绝 protected/internal 路径，并在调用 provider 前校验目标 resolved physical path 仍位于 `/workspace`，且反推回 workspace virtual path 后不是 protected/internal target；symlink escape 或 provider-specific security failure 会统一归一成 harness path policy error。Local provider listing 对 workspace 内部 symlink 保留链接自身的 virtual path，不把 entry path 改写成目标文件路径。新 harness 链路不再引入 `/mnt/user-data` alias；旧 thread artifact / upload helper 若仍出现该路径，只能作为待迁移的非 harness 历史边界存在。

带 sandbox 工具的 ReactSubagent 会通过 `backend/src/agents/harness/context_assembly.py` 接收同一份 bounded harness context：默认 user payload 中包含 `_harness_context(schema=wenjin.harness.context_bundle.v1)`，system prompt 也会追加 `Harness context bundle`。因此即使 skill 使用自定义 `user_template`，模型仍能看到任务摘要、workspace 类型、`/workspace/datasets`、`/workspace/scripts`、`/workspace/outputs`、`/workspace/reports`、protected paths、`/workspace/tmp/tasks/.harness/outputs/**` 内部路径规则、recent execution evidence 和 context budget。`build_agent_workspace_contract()` 还会提供机器可读 `path_classes`：workspace、datasets、scripts、artifacts、scratch、task_scratch、runtime、protected、internal、guidance 和 `task_scratch_root=/workspace/tmp/tasks`；同时提供 `operation_policy`，说明 direct write tools、manifest update tools 和 internal output ref 读取工具。`context_assembly.py` 将其投影为 `sandbox.path_classes`、`sandbox.operation_policy`、`sandbox.task_scratch_root`、当前成员的 `sandbox.task_scratch_path` / top-level `task_scratch_path`、`sandbox.task_contract(schema=wenjin.workspace_sandbox.task_contract.v1)` 和 `sandbox.guidance_paths`。底层 `build_workspace_task_contract()` 是 task-scoped 真实路径来源，包含 scratch path、内部 output-ref root、manifest path 和 reviewable artifact roots；`build_agent_workspace_task_contract()` 是安全投影，context 和 Lead-owned run job metadata 都使用它，不暴露 raw `output_ref_root`。可恢复 output refs 只通过 `sandbox_execution_summary.output_refs` / `output_ref_recovery` 进入后续上下文；成员实际读取过的 explicit output refs 则以 `member_execution_transcript.output_refs_read` 记录为 bounded telemetry，供后续 context 和 `workflow_trace` 审计判断是否复用过外部化证据。`task_scratch_path` 由 task contract 安全生成，例如 `/workspace/tmp/tasks/{execution_id}/{invocation_id}`；它是当前执行/成员的普通临时工作区，不是 `.harness` internal refs，也不是用户可审阅 artifact root。context 从 path classes 派生 `workspace_roots`、workspace file summary 可见根和上游产物过滤规则，让成员按稳定 schema 判断写稿、数据、脚本、产物、任务临时文件、manifest 更新工具和内部引用位置，而不是解析提示词或依赖硬编码目录列表。上游 sandbox output payload 中的安全 task scratch 会单独投影为 top-level `scratch_refs[]`，供后续成员接续实验；这些 refs 不会进入 `upstream_artifact_candidates`、workspace file summary 或用户 review item，且 raw `sandbox_outputs` / `upstream_sandbox_outputs` 会从 bundle 的 `task` 字段剥离，避免非 contract path 绕过过滤。这个 bundle 也是团队成员执行上下文 schema，top-level 提供 `capability_goal`、`member_role`、`allowed_tools`、`workspace_roots`、`task_scratch_path`、`scratch_refs`、`search_ignored_names`、最新 file-change / sandbox-execution / reproducibility summaries、`member_execution_transcript`、`harness_replan_signals` 和 `upstream_artifact_candidates`，让工作流质量优化依赖稳定字段而不是散落 prompt 文本。Lead runtime 加载 workspace source context 时统一读取 DataService `list_sources_page().items[].assets`，把已经显式位于 `/workspace/datasets/**` 的 source asset 投影为 `workspace_file_summary.dataset_provenance`；context assembly 仍会二次过滤，只保留 sandbox contract datasets path class 下的安全路径和 bounded provenance 字段，并过滤 protected/internal workspace refs；`sandbox.run_python` 再把这份安全 provenance 同步到 sandbox 内 dataset manifest，确保后续长程实验和新成员可以从文件系统看到同一份数据来源清单。文件系统规范不得只停留在 provider 建目录层，必须进入 tool-using agent 的运行上下文和 DataService sandbox job metadata。

工具大输出统一写入 `workspace_harness_output_ref_path()` 生成的 `/workspace/tmp/tasks/.harness/outputs/{execution_id}/{node_id}/{invocation_id}/` 内部路径，模型只接收 bounded preview、refs、`truncated` 和 `externalized` 标记。当前已覆盖 `sandbox.read_file` 的大文件读取、Lead-owned `sandbox.run_python` 的 stdout/stderr，以及 `sandbox.write_file` / `sandbox.str_replace` / `sandbox.apply_patch` 的大 unified diff；完整内容留在 workspace sandbox 内，明确的 `/workspace/tmp/tasks/.harness/outputs/**` output ref 可以通过 `sandbox.read_output_ref(output_ref,start_line,end_line,max_chars)` 只读回看，且仍受同一 output budget 约束。output-ref 路径的 execution/node/invocation segment 由 `workspace_layout.py` 统一清洗，不能在 output budget、diff tracker 或 runner 中各自拼接。`sandbox.read_file(max_chars=...)`、`sandbox.glob(max_matches=...)` 和 `sandbox.grep(max_matches=...)` 的调用参数只能进一步收窄 policy 上限，不能放大 `read_max_chars` 或 `search_max_matches`；externalized output 的 head/tail 正文片段同样会被 fallback budget 钳制，不能通过 `preview_head_chars` / `preview_tail_chars` 重新放大模型可见内容。`sandbox.list_dir` 的 preview 与 structured `entries` 共用 `search_max_matches` 上限，并返回 `total_entries` / `returned_entries`；`sandbox.glob` / `sandbox.grep` 的 `matches` 同样受上限约束，并返回 `returned_matches` / `match_limit`，避免大目录或大搜索结果把完整列表塞进 tool JSON。`sandbox.grep` 还会按 `grep_max_file_bytes` / `grep_max_line_chars` 控制扫描成本，跳过超大文件、二进制文件和超长行，并在 payload 中返回 `scanned_files` 与 skipped 计数；非法 regex 返回 recoverable JSON tool error，不扫描文件、不中断 agent loop，并在 tool record / completed event 上投影 `recoverable_error` 与 `error_code`。普通 tool 输出通过 `output_refs` 投影到 tool call record 与 `execution.harness.output_externalized` 事件；文件改动 diff 通过 `file_change.diff_output_refs`、`diff_externalized`、`diff_truncated` 投影到 tool call record、`execution.harness.file_change` 事件和节点摘要。实现上不得把完整 stdout/stderr、大文件内容、大目录列表、大搜索结果或大 diff 重新塞进 model-visible tool payload；后续读取 output ref 也必须返回 bounded preview。

Lead-owned `sandbox.run_python` 在同一个 workspace lease 内发现用户可审阅产物：扫描 `/workspace/outputs` 与 `/workspace/reports`，但候选项和可反解的 resolved target 都必须先通过 `is_user_reviewable_workspace_artifact_path()`，因此 protected `.env`、internal output refs、README、manifest、`.gitkeep` 这类 guidance 文件，以及指向 protected/internal target 的 symlink 都不会进入 `generated_artifacts[]`。候选项字段包括 `schema=wenjin.sandbox.generated_artifact_candidate.v1`、`path`、`root`、`artifact_kind`、`mime_type`、`size`、可选 `content_hash`、`review_surface=sandbox_artifact` 与 `materialization_status=candidate`；若 artifact manifest 里存在同 path 安全条目，discovery 会用 title、description、artifact_kind、source_script、dataset_paths 和 notes 丰富候选项。ReactSubagent harness adapter 会把这些候选项连同 `sandbox_job_id` / `sandbox_environment_id` 投影到 tool call record 和 `execution.harness.tool_call.completed` 事件，前端/运行历史不需要解析 raw tool JSON。普通 LangGraph runtime 与 TeamKernel 都会把可信候选注册为 DataService `workspace_asset(storage_backend=sandbox)` 和 `sandbox_artifact`，由 DataService 自动生成 `target_domain=sandbox,target_kind=sandbox_artifact` 的 review item；用户接受后才 mark materialized。写作成员的 `prism_file_change` declaration 走同一 DataService review item 面，而不是通过 sandbox artifact 物化；mock TeamKernel E2E 必须同时覆盖 library evidence、sandbox artifact evidence 和 Prism writing evidence。

`sandbox.write_file`、`sandbox.str_replace` 与 `sandbox.apply_patch` 会记录 hash + unified diff 的 `file_change`，并由 ReactSubagent harness adapter 投影到 tool call record、`execution.harness.tool_call.completed` 和专门的 `execution.harness.file_change` 事件。`sandbox.apply_patch` 是结构化多文件 patch 工具，只支持 `replace` 和 `write` edit，会先校验全部 edit、唯一匹配和路径边界，再执行任何 mutation；它返回 `file_changes[]`，供节点聚合多文件净变更。写工具在实际 mutation 前必须同时具备 `filesystem.write` 与 `filesystem.diff`，底层 `SandboxFileTools` 也会执行同一边界校验，不能只依赖上层 policy resolver。小 diff 保留在 `unified_diff`；大 diff 的 `unified_diff` 是 head/tail 预览，完整 diff 写入 `/workspace/tmp/tasks/.harness/outputs/**`，并通过 `diff_output_refs` 保留引用。Lead runtime 与 TeamKernel 会从这些 tool call record 聚合 path/hash 级净文件变更摘要，写入 `ExecutionNodeRecord.node_metadata.harness.file_change_summary`，schema 为 `wenjin.harness.file_change_summary.v1`，摘要中的 compact diff 同样保留大 diff refs。failed / recoverable harness tool calls 也会聚合到 `ExecutionNodeRecord.node_metadata.harness.tool_failure_summary`，schema 为 `wenjin.harness.tool_failure_summary.v1`；`sandbox.run_python` 的 manifest、failure classification 与 generated artifact evidence 还会聚合到 `ExecutionNodeRecord.node_metadata.harness.sandbox_execution_summary`，schema 为 `wenjin.harness.sandbox_execution_summary.v1`，包含 Python run 数、失败数、recoverable failure 数、sandbox job/environment ids、failure codes 和 generated artifact count。`ExecutionNodeRecord.node_metadata.harness.member_execution_transcript(schema=wenjin.harness.member_execution_transcript.v1)` 是成员级执行记忆投影，汇总工具调用数、工具名、失败工具、变更路径、sandbox job/environment ids、安全 scratch refs、读取过的 explicit output refs、生成产物数、token usage 和 duration；它只来自现有 tool calls，不保存 raw args、脚本、stdout/stderr、protected 路径或任意 internal path，`output_refs_read` 只保留 `/workspace/tmp/tasks/.harness/outputs/**` 这类可通过 `sandbox.read_output_ref` bounded 回看的 refs，也不新增 run table。`ExecutionNodeRecord.node_metadata.harness.run_journal_summary(schema=wenjin.harness.run_journal_summary.v1)` 是给 RunView/历史恢复读取的产品化成员进度摘要，优先表达工具异常、实验修订、生成产物、文件更新等结论，不要求前端解析 raw tool JSON。每条 `execution.harness.*` event envelope 也可携带 `journal(schema=wenjin.harness.journal_event.v1)`，作为现有 execution event path 上的精简进度条目；不新增 run journal table 或第二套 stream。`python_exit_nonzero`、`sandbox_queue_timeout`、`tool_forbidden` / `tool_unknown` 会进一步生成 `wenjin.harness.replan_signal.v1`，TeamKernel 只把这些信号写入 blackboard 和 `harness_replan_signal` quality gate：用户代码非零退出可触发一次同模板修订，queue timeout 与 forbidden/unknown tool 只给 warning/stop，不通过重复招募绕过权限。`research_evidence_required` 失败则通过现有 `QualityGateResult.required_fixes[*].repair_context` 投影 `quality_repair_context(schema=wenjin.team.quality_repair_context.v1)`，只包含 missing surfaces、安全 output refs 和短 required actions，下一轮成员从 `upstream_context.quality_repair_context` 读取，不新增运行表、stream 或 raw evaluator payload。citation/source quality gates 还会把 high-risk `citation_key_audit`、`missing_sources`、`fabrication_risks` 与 `bibtex_projection_notes` 规范化为 `wenjin.quality.citation_source_audit_finding.v1`，挂在现有 `QualityGateResult.findings[*].citation_source_audit` 下；该 evidence 只保留 bounded claim/message、当前 workspace 可信 source/citation refs 或 unknown refs、risk/severity 和 suggested action，不引入新表、新 stream 或 raw member-output UI 解析。这些都供 Lead、Run History 和后续 replanning 使用，不需要解析 raw tool JSON，也不是新的 harness run table。

`sandbox_execution_summary` 的 `generated_artifact_count` 只统计通过 workspace layout 判定的用户可审阅 `/workspace/outputs/**` 或 `/workspace/reports/**` 产物，不统计 protected/internal/non-artifact 候选。它还会吸收 `execution_lifecycle(schema=wenjin.sandbox.execution_lifecycle.v1)` 的 bounded 字段：lifecycle count、job statuses、exit codes，以及明确可通过 `sandbox.read_output_ref` 回看的 `/workspace/tmp/tasks/.harness/outputs/**` output refs。context assembly 只在这个 summary 字段内保留这些 explicit output refs，其他 protected/internal/non-output paths 继续被过滤；若存在 output refs，还会生成 `output_ref_recovery(schema=wenjin.harness.output_ref_recovery.v1)`，把 read tool、refs 和“先读 ref 再决定是否重跑”的恢复建议放到 context 顶层。

`member_execution_transcript.billing.credits_charged` 是从现有 tool call billing payload 汇总出的展示/上下文字段；积分扣费事实源仍是 DataService credit reservation / transaction，不由 harness transcript 回写或重算。

Sandbox 依赖安装由 Lead-owned runtime 负责，不由 subagent 自行拼装系统命令。`sandbox_python` 只传 `dependency_hints`；runtime 在 workspace lease 内确保 Python venv 存在、按受控 pip command 自动安装 hints、遇到 `ModuleNotFoundError` 时最多安装缺失包并重试一次。deterministic `sandbox_python` subagent 与 React harness `sandbox.run_python` 必须暴露同一组 harness metadata surfaces：`execution_manifest`、`reproducibility_manifest`、`execution_lifecycle`、output refs、`generated_artifacts`，以及可用时的 `failure_classification`，确保 RunView、quality gate 和历史恢复不按入口分叉。`sandbox.run_python` 的 `script_name` 在 harness 边界和 runner 内复用同一 sanitizer：清理路径穿越片段和隐藏/protected-looking 前缀，保留 `.py` 后缀，并用 `is_user_editable_workspace_path("/workspace/scripts/{safe_name}")` 校验最终脚本路径；Lead-owned 直接写脚本不能比 model-facing 文件工具拥有更宽路径边界。安装 job 记录为 `operation=install_dependencies` 且 `billable=false`，实际 Python run / smoke check job 保持 billable 并通过 credit reservation 结算。安装网络只通过 `package_index_only` profile 开启，普通运行默认 `none`。每次 Python run 都会先创建 `/workspace/tmp/tasks/{execution_id}/{node_id}`，写入 `.gitkeep` 保证目录存在，并以该目录作为 sandbox command cwd 注入 `WENJIN_TASK_SCRATCH` 与 `WENJIN_WORKSPACE_ROOT`；Local provider 会把这些 `/workspace` env 值映射到宿主物理路径执行并把 stdout/stderr 反解回 `/workspace`，Docker provider 则使用同一虚拟 cwd/env 进入容器。Docker provider 会在首次 acquire 时按 Wenjin managed/kind label 清理异常遗留的 ephemeral exec containers；cleanup 失败不会阻塞本次 acquire，但不会标记为已完成，后续 acquire 会重试，避免一次 Docker daemon 抖动让当前进程生命周期内永久跳过 orphan reconciliation。每次 harness `sandbox.run_python` 返回都会携带 `execution_manifest(schema=wenjin.harness.run_python.execution_manifest.v1)`，记录 workspace/execution/node/invocation、safe script path、task scratch path、dependency hints、sandbox job/environment、network profile 和 effective timeout；`reproducibility_manifest` 会携带 retry count、task scratch path、已同步 dataset provenance 的 bounded 路径/source/hash 摘要；`experiment_narrative(schema=wenjin.harness.run_python.experiment_narrative.v1)` 会携带 status、script path、task scratch path、dataset/artifact paths、dependency names、command risk 和 next actions，帮助后续成员接续长程实验；`report_markdown` 会追加用户可读的 Experiment narrative 段落。用户代码非零退出会保留 runner payload，不再在 harness tool 层丢失 stdout/stderr/exit_code，并附加 `failure_classification(schema=wenjin.harness.run_python.failure_classification.v1)`、`error_code=python_exit_nonzero` 和 bounded Recovery guidance，供 Lead、Run History 和 replanning 判断。

Agent harness 是 Lead/subagent 的工具执行层，不是新的 agent framework。当前第一版内置 `sandbox.list_dir`、`sandbox.glob`、`sandbox.grep`、`sandbox.read_file`、`sandbox.read_output_ref`、`sandbox.write_file`、`sandbox.str_replace`、`sandbox.apply_patch`、`sandbox.register_dataset`、`sandbox.register_artifact` 和 `sandbox.run_python`；`sandbox_python` / `sandbox_exec` 在 TeamKernel 与 ReactSubagent 入口 canonicalize 为 `sandbox.run_python`。Capability policy 是最大权限包络，skill/template 只能收窄；未知或禁用工具显式失败，不降级成 plain LLM。文件工具只允许 `/workspace` 下路径，并复用 workspace layout 的 protected/internal/guidance path 分类；默认 protected paths 覆盖 `.git/**`、根目录和子目录 `.env` / `.env.*`、`*.pem`、`*.key`、`.wenjin/env/**`、`.wenjin/cache/**` 与 `.wenjin/manifest.json`；读/list/search 工具在实现边界也要求 `filesystem.read`，写工具、`sandbox.apply_patch`、`sandbox.register_dataset` 和 `sandbox.register_artifact` 要求 `filesystem.write` / `filesystem.diff`，不能只依赖上层 registry 过滤；list/search 不返回 protected/internal 路径，直接读写也会拒绝 protected/internal 路径、layout guidance/manifest 路径，以及 resolved target 落到 workspace 外、protected 或 internal 的 symlink target；manifest 更新只能走结构化 register 工具；写入会记录 hash + bounded unified diff；大输出与大 diff 通过 `backend/src/agents/harness/output_budget.py` 外部化到 workspace harness outputs。ReactSubagent 的 harness adapter 会把单次工具执行异常降级成 bounded JSON error result，记录 `status=failed` tool call 并发布 `execution.harness.tool_call.failed`，让模型可以继续选择替代工具；TeamKernel 会把 failed / recoverable tool calls 聚合成 `node_metadata.harness.tool_failure_summary`，让上层可以判断局部失败与恢复情况；`sandbox.run_python` 的 manifest 与 failure classification 会投影到 tool record metadata 和 completed event metadata，运行详情不需要解析 raw JSON；LangGraph 控制流信号和 loop guard hard stop 仍保持硬停止。重复相同工具调用达到 warning 阈值时发布 `execution.harness.loop_warning`，visibility 为 `team_visible`，不在 assistant tool call 与 tool result 之间插入任何消息。

`backend/src/agents/harness/command_audit.py` 已提供 argv-first command audit 与 `policy_decision(schema=wenjin.harness.command_policy_decision.v1)`；Lead-owned `run_python`、`install_dependencies` 和 smoke check sandbox jobs 在创建 job 前执行 allow/forbid guard，并把 `metadata.command_audit` 写入 DataService sandbox job。该 decision 对象统一记录 `operation`、`decision`、`risk_level`、`reason`、`network_profile`、`billable`、`blocked_before_job` 和 bounded `command_preview`：依赖安装是 `operation=install_dependencies,billable=false,network_profile=package_index_only`，实际 Python run 与 smoke check 是 billable。command audit 会在任何 job 创建前 fail-close null byte 与超长 command payload，并把 argv、shell command、cwd、env、operation、network profile 和 preview 做 bounded metadata sanitization；`invalid_command_input` 不会继续落入 runner、Docker 或 DataService raw metadata。Lead-owned `sandbox.run_python` 还会把 `execution_lifecycle(schema=wenjin.sandbox.execution_lifecycle.v1)` 写入 sandbox job create/final update metadata 并带回 harness payload/tool record metadata；它只记录 queued/final status、job/environment、runtime/provider image、command preview/argv、cwd、env keys、network profile、timeout、exit code、explicit internal output refs 和用户可审阅 artifact count，不记录 raw stdout/stderr、脚本正文、普通 workspace 文件、protected path 或 non-output internal refs。harness `sandbox.run_python` 会把 run/install 审计带回 payload、tool record、`execution.harness.tool_call.completed` 和 `execution.harness.command_audit(team_visible)`。通用 `sandbox.run_command` 仍未启用，需完整 DataService command policy、输出预算和事件投影评审后再加。
...
- Workspace memory runtime 使用 WorkspaceMemory DataService client：后台隐藏 Markdown memory document 和 workspace-context upload memory note 不打开 DB session，不 reset DB engine，不执行 request-time commit/rollback；旧 `user_memory_service`、`memory_compaction` 和默认 Celery memory capture 不再作为运行态路径存在
...
- Prism file workspace projection 和 workspace activity projection 只读取 canonical asset metadata，不再读取 `legacy_kind` / `legacy_parent_id` / `legacy_version`
