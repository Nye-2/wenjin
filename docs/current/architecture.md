...
`sandbox.write_file` 与 `sandbox.str_replace` 会记录 hash + unified diff 的 `file_change`，并由 ReactSubagent harness adapter 投影到 tool call record、`execution.harness.tool_call.completed` 和专门的 `execution.harness.file_change` 事件。当前这是 per-tool 变更事实源；后续若要做类似 Codex turn diff 的净变更摘要，应从这些 record/event 聚合，不新增 harness run table。
