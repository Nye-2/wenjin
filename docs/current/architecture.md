...
Lead-owned `sandbox.run_python` 在同一个 workspace lease 内发现用户可审阅产物：扫描 `/workspace/outputs` 与 `/workspace/reports`，但候选项必须先通过 `is_user_reviewable_workspace_artifact_path()`，因此 protected `.env`、internal output refs、README、manifest 和 `.gitkeep` 这类 guidance 文件不会进入 `generated_artifacts[]`。候选项字段包括 `schema=wenjin.sandbox.generated_artifact_candidate.v1`、`path`、`root`、`artifact_kind`、`mime_type`、`size`、可选 `content_hash`、`review_surface=sandbox_artifact` 与 `materialization_status=candidate`；若 artifact manifest 里存在同 path 安全条目，discovery 会用 title、description、artifact_kind、source_script、dataset_paths 和 notes 丰富候选项。ReactSubagent harness adapter 会把这些候选项连同 `sandbox_job_id` / `sandbox_environment_id` 投影到 tool call record 和 `execution.harness.tool_call.completed` 事件，前端/运行历史不需要解析 raw tool JSON。普通 LangGraph runtime 与 TeamKernel 都会把可信候选注册为 DataService `workspace_asset(storage_backend=sandbox)` 和 `sandbox_artifact`，由 DataService 自动生成 `target_domain=sandbox,target_kind=sandbox_artifact` 的 review item；用户接受后才 mark materialized。写作成员的 `prism_file_change` declaration 走同一 DataService review item 面，而不是通过 sandbox artifact 物化；mock TeamKernel E2E 必须同时覆盖 library evidence、sandbox artifact evidence 和 Prism writing evidence。
