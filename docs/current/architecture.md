...
50. TeamKernel 成员上下文必须把 capability policy 的 research evidence contract 投影为 `research_evidence_requirements(schema=wenjin.team.research_evidence_requirements.v1)`，包含 `quality_gate=research_evidence_required`、ordered required surfaces、runtime-enforced surfaces 和 bounded guidance。Harness context budget policy 统一在 `backend/src/agents/harness/context_budget_policy.py`，`context_assembly.py` 只负责构造 bounded bundle；budget policy 必须按 required runtime surface 保护对应 evidence summary：`workflow_trace` 保护成员执行痕迹和安全 scratch refs，`output_ref_reuse` 保护 sandbox execution summary / output-ref recovery / member transcript，`experiment_interpretation` 保护 experiment interpretation 与 reproducibility summary，`statistical_robustness` 保护 robustness 与 reproducibility summary；只有在非保护上下文和结构性 sandbox/workspace 描述已压缩后，才允许把这些 required evidence summary 降级为空。
...
57. Academic Harness v2 在 v1 envelope 内增加四个结构化学术事实源：`research_brief.v1`、`claim_inventory.v1`、`evidence_packet.v1` 和 `academic_workspace_map.v1`。TeamKernel run start 从已有 bounded `workspace_data` 生成 academic workspace map summary，再构建 research brief；初始成员通过 member context 接收 brief/map，后续成员通过 enriched `ResearchStateV1` 接收 compact claim/evidence/artifact/blocker handoff。SCI first-wave skills 的 Prompt Pack v2 要求把可复用论断写入 `expert_report.claim_inventory`，把来源/引用审计/数据集/脚本/产物/Prism/user material/expert judgment 写入 `expert_report.evidence_packet`；缺证据必须显式标记 `insufficient_evidence` 或 warning/blocker。`claim_evidence_alignment` eval 会把 high/critical claim warning 视为失败。该 v2 仍不引入第二 runner、第二 event stream、embedding router、权限 profile 或 review packet commit path。
