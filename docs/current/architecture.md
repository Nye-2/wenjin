...
48. Research-task eval 是 deterministic harness 质量基础设施，不调用模型、不创建第二套运行态。`backend/src/agents/harness/research_task_eval.py` 只审计 `TaskReport`、review items、hydrated node metadata 和 TeamKernel `runtime_state.quality_gates` 是否覆盖文献证据、sandbox 实验复现、写作审阅、`workflow_trace` 成员执行痕迹 surface，以及可选 `citation_strength` / `experiment_interpretation` / `paper_relevance` / `statistical_robustness` / `writing_semantic_preservation` outcome-quality surfaces；它用于 release gate / mock E2E，也被 TeamKernel quality gates 复用来检查 capability 声明的 runtime 可验证 surfaces，但不替代人工 review 或创建新的执行 runner。TeamKernel 的 `research_evidence_required` gate 只评估可从当前 `ExecutionNodeRecord.node_metadata.harness` 证明的 surfaces（`workflow_trace`、`output_ref_reuse`、`citation_strength`、`experiment_interpretation`、`paper_relevance`、`statistical_robustness`），失败时进入 `revise_existing`；依赖最终 `TaskReport.outputs` / `review_items` 的 literature、experiment、writing 和 Prism semantic preservation 仍保留在 final report / release gate eval 中，避免在 review item staging 前误卡运行。citation/source audit evidence 必须来自 bounded `citation_source_audit` findings，明显 fabricated / missing / unsupported 或 high/critical/blocking severity 的 refs 不得作为文献证据通过；`citation_strength` 比文献结构覆盖更严格，只接受 supported/verified/low-risk 等强证据，weak-only refs 会失败。`paper_relevance` 读取现有 `node_metadata.harness.paper_relevance_summary`，要求至少一个带 `source_id` 或 `citation_key` 的 topic-aligned ref，且不能存在 off-topic ref；它不运行模型 judge、不自行检索、不把弱相关文献当作贴题证据。`experiment_interpretation` 读取现有 `node_metadata.harness.experiment_interpretation_summary` 和 `reproducibility_summary`，要求方法摘要、指标、已验证结果、限制、artifact 和 dataset path 同时存在并与可复现实验路径对齐；它不读取 raw stdout/stderr、不运行模型 judge、不新增运行态。`statistical_robustness` 读取现有 `node_metadata.harness.statistical_robustness_summary` 和 `reproducibility_summary`，要求统计方法、样本量、指标、至少一项通过的稳健性检查、限制、artifact 和 dataset path 同时存在，且 artifact/dataset path 与可复现实验路径对齐；critical/high/blocking 级 failed robustness check 不得通过。`writing_semantic_preservation` 读取 canonical Prism review item 的 `preview.content_contract` 和 `preview.semantic_contract`，要求每个 Prism 改稿都有低风险结构语义保护证据，不能出现 high-risk、结构无效、claim/citation/equation/table preservation 失败；它只做 bounded structural heuristic / upstream contract 审计，不读取全文原稿、不运行模型 judge、不绕过 review-first flow。`workflow_trace` 只读取 `node_metadata.harness.member_execution_transcript`，要求至少存在已完成工具活动，并汇总工具、变更、sandbox ids、scratch refs、usage、billing 和 duration；它不读取 raw tool JSON、不重算扣费、不新增运行态。TeamKernel 与普通 Lead runtime 都必须把 `prism_file_change` output declaration stage 成 canonical Prism review item，不能把写作 evidence 留在成员 raw output 或普通 room output 中；`.tex` review item 还必须在 `preview.content_contract` 中提供 bounded 结构摘要，`main.tex/project:main` 必须是完整 LaTeX document，并在 `preview.semantic_contract(schema=wenjin.prism.semantic_contract.v1)` 中提供 bounded 语义保护摘要。
