...
5. **Quality gates now include workflow trace and early outcome-quality scoring.**
   ...
   Literature, experiment, Prism writing evidence, and team `workflow_trace` now have deterministic structure checks. The workflow trace check consumes existing `member_execution_transcript` metadata so a SCI workflow can fail release-gate evaluation when review items exist but no team member transcript shows completed tool activity. `citation_strength` checks strong citation/source support. `experiment_interpretation` checks that sandbox-backed results have method, metric, verified-result, limitation, artifact, and dataset evidence aligned with reproducibility metadata. `paper_relevance` checks that cited/selected papers include topic-aligned refs and no off-topic refs. `statistical_robustness` checks method, sample size, metrics, passed robustness checks, limitations, and artifact/dataset alignment while rejecting critical failed checks. `writing_semantic_preservation` checks Prism review items for bounded structure/semantic-preservation contracts before they count as low-risk writing evidence. `writing_academic_style` now checks Prism review items for bounded academic-register signals, minimum style score, and obvious AI-meta/casual/vague anti-patterns without exposing pending manuscript content or running a model judge. When a writer provides `style_delta`, the canonical Prism review payload carries it through `PrismFileChangeUpsertPayload.academic_style_contract`; Prism DataService stores only an allowlisted bounded contract, projection recomputes the final delta, and research eval requires a positive consistent score delta. Before/after raw text is not stored in the contract, and legacy `prism.file_changes[]` does not expose quality contracts. SCI sandbox-heavy capability seeds now declare `research_evidence.required_surfaces`, `LeadAgentRuntime._capability_policy()` projects that contract, and `required_surfaces_from_capability_policy()` is the narrow evaluator reader used by the mock SCI E2E to derive required surfaces from capability data instead of hard-coding them in the test. TeamKernel now also enforces capability-required, node-metadata-verifiable research surfaces such as `workflow_trace`, `experiment_interpretation`, `citation_strength`, `paper_relevance`, `statistical_robustness`, and `output_ref_reuse` through the same deterministic evaluator; report/review-item-dependent surfaces such as literature, experiment, writing, Prism semantic preservation, and Prism academic style remain final-report/release-gate checks. Failed `research_evidence_required` gates now include a bounded `quality_repair_context(schema=wenjin.team.quality_repair_context.v1)` with missing surfaces, safe `sandbox.read_output_ref` refs and short required actions, so the next member receives repair guidance without raw stdout/stderr/tool args or raw evaluator payload. Remaining gap: calibrating style scoring/delta production on real edited manuscripts and deciding which quality-evidence fields should be reviewer-facing versus agent-only.

   Capability-declared requirements are now also projected into every recruited member brief as `research_evidence_requirements(schema=wenjin.team.research_evidence_requirements.v1)`, so the member sees the ordered surfaces and runtime-enforced subset before execution. Harness context budgeting treats required runtime evidence as protected: for example, an `experiment_interpretation` requirement keeps `experiment_interpretation_summary` and `reproducibility_summary` ahead of generic sandbox/workspace verbosity, and only drops them after all recoverable context has been compressed.
...
1. Run one real SCI workflow through the runtime `research_evidence_required` gate with `workflow_trace`, `experiment_interpretation` and optional `output_ref_reuse` required, then tune prompts/tools from the failures.
2. Tune prompt/tool guidance from real runs where the member receives `research_evidence_requirements` but still fails to produce the required evidence, especially output-ref reuse and experiment interpretation.
3. Calibrate `writing_academic_style` and optional `style_delta` production on real Prism rewrite tasks, then decide which style/semantic evidence belongs in reviewer-facing UI.
