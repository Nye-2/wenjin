...
16. Runtime boundary convergence gate：
  ...
  - Prism adapter metadata 必须使用 canonical `source_metadata`，不得对外暴露 `legacy_metadata`
  ...
  - Artifact follow-up / rerun action state 必须需要显式 mission params 或 source artifact；不得用 workspace description/name 或“未命名任务”合成 goal
