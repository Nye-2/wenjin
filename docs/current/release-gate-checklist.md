...
2026-06-10 最新增量验证：Native harness release gate 已进入 core checks；native harness gate command 306 passed，覆盖 harness filesystem、sandbox file tools、command audit、policy/registry、output budget/loop/diff、research-task eval（含 `workflow_trace` 成员执行痕迹、可选 `citation_strength` 引文强度、`experiment_interpretation` 实验解释和 `paper_relevance` 文献贴题度）、Prism writing evidence eval、Prism review projection structure contract、LangChain adapter、harness context assembly、workspace sandbox metadata、workspace layout、sandbox artifact discovery、citation/source audit、team quality gates、mock sandbox E2E。mock TeamKernel E2E 已要求文献、sandbox 实验、Prism 写作和 workflow trace 四个 evidence surface 同时闭环；deterministic eval 还会要求现有 `member_execution_transcript` 证明团队成员完成过工具活动，并通过安全 task scratch ref 证明 sandbox 长程任务上下文连续。`citation_strength` 是更严格的 outcome-quality surface：weak-only refs 不通过，supported/verified/low-risk refs 才能证明引文支撑强度。`experiment_interpretation` 要求方法、指标、已验证结果、限制、artifact 和 dataset 证据，并与可复现摘要路径对齐。`paper_relevance` 要求至少一个 topic-aligned source/citation ref，off-topic refs 不通过。
