...
16. Runtime boundary convergence gate：
  ...
  - Gateway common deps 不得导出 `get_db`；ExecutionService、TaskStore、SkillResolver 不得接受 DB/session constructor 或保存 DB session state
17. 统一门禁命令（发布前需要运行）：
