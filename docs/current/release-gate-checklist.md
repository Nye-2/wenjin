...
16. Runtime boundary convergence gate：
  ...
  - Reference Library router、BibTeX export/validation、Prism `refs.bib` sync 必须通过 Source/Asset/Prism DataService client，不得注入 `get_db`、`AsyncSession` 或保存 `self.db`
