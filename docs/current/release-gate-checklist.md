...
2026-06-09 最新增量验证：Native harness release gate 已进入 core checks；release-gate service/config suite 12 passed；native harness gate command 189 passed，覆盖 harness filesystem、sandbox file tools、command audit、policy/registry、output budget/loop/diff、research-task eval、Prism writing evidence eval、LangChain adapter、harness context assembly、workspace sandbox metadata、workspace layout、sandbox artifact discovery、citation/source audit、team quality gates、mock sandbox E2E。mock TeamKernel E2E 已要求文献、sandbox 实验和 Prism 写作三个 evidence surface 同时闭环。
