...
2026-06-09 最新增量验证：Native harness release gate 已进入 core checks；native harness gate command 295 passed，覆盖 harness filesystem、sandbox file tools、command audit、policy/registry、output budget/loop/diff、research-task eval（含 `workflow_trace` 成员执行痕迹）、Prism writing evidence eval、Prism review projection structure contract、LangChain adapter、harness context assembly、workspace sandbox metadata、workspace layout、sandbox artifact discovery、citation/source audit、team quality gates、mock sandbox E2E。mock TeamKernel E2E 已要求文献、sandbox 实验、Prism 写作和 workflow trace 四个 evidence surface 同时闭环；deterministic eval 还会要求现有 `member_execution_transcript` 证明团队成员完成过工具活动，并通过安全 task scratch ref 证明 sandbox 长程任务上下文连续。
