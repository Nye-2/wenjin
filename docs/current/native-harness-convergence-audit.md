...
- **bounded output**：大文件读取、搜索、Python stdout/stderr 和 diff 只给模型 bounded preview，完整内容外部化到 `/workspace/tmp/tasks/.harness/outputs/**`，并作为内部 refs 进入 tool record / event；明确 output ref 可由 `sandbox.read_file` 受限只读回看，列表、搜索、写入和 artifact 注册仍不可见。
