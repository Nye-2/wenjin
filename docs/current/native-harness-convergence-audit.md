...
- **运行证据而非文本猜测**：`execution_manifest`、`reproducibility_manifest`、`experiment_narrative`、`failure_classification`、`sandbox_execution_summary`、`reproducibility_summary`、`run_journal_summary` 都挂回 harness payload / `ExecutionNodeRecord.node_metadata.harness`，RunView 不解析 raw tool JSON；`report_markdown` 已包含用户可读的 Experiment narrative、Reproducibility 段落和依赖安装失败恢复建议。
