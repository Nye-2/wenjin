...
- **bounded output**：大文件读取、搜索、Python stdout/stderr 和 diff 只给模型 bounded preview，完整内容外部化到 `/workspace/outputs/harness/**`，并作为内部 refs 进入 tool record / event；明确 output ref 可由 `sandbox.read_file` 受限只读回看，列表、搜索、写入和 artifact 注册仍不可见。
...
- 2026-06-09 external pattern gap matrix + output-ref readability slice:
  ...
  - `backend`: `.venv/bin/python -m pytest tests/agents/harness/test_sandbox_file_tools.py::test_direct_file_tools_allow_read_only_internal_harness_output_refs -q` -> RED while `sandbox.read_file` still rejected every internal path, then passed after direct `/workspace/outputs/harness/**` output refs became bounded read-only.
