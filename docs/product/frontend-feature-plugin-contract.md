...
### 1.2 解析 feature follow-up / rerun action
...
- feature 启动与恢复以 thread orchestration + `FeatureIngressService` 为准。
- 返回给前端用于订阅、恢复、提交的 canonical 标识始终是 `execution_id`。

## 4. 交互约束

1. Feature 按后端下发动态渲染，不做 workspace 类型硬编码按钮列表。
2. 功能执行后统一汇聚到 `ExecutionRecord`，并由 `ComputeSession` 提供工作台 shell。
