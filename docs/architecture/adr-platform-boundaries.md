...
AcademiaGPT-V2 目前已形成统一任务执行链路，但仍需要持续约束各层职责，避免路由层回流业务逻辑、避免任务层与 HTTP 细节耦合，保证可测试性和可演进性。
...
|---|---|---|---|
| Router | `backend/src/gateway/routers/`、`backend/src/thesis/api.py` | HTTP 协议适配、鉴权依赖注入、请求参数校验、返回模型 | 直接编排积分扣费、直接执行业务补偿、直接做跨服务流程控制 |
...
5. `/api/thesis/*` 与 `academic` 路由属于兼容层，新增能力不得继续投入到弃用面。
...
| `/api/tasks/*` | Active，统一任务状态与流式进度 |
| `/api/thesis/*` | Deprecated，兼容保留（Sunset: 2026-05-01） |
| `academic` tag routes | Deprecated，兼容保留（Sunset: 2026-05-01） |
