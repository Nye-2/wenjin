...
| Application Handler | `backend/src/application/handlers/` | chat turn、命令路由、用户交互分支编排 | 处理 HTTP 细节、直接提交 feature task |
| Application Service | `backend/src/application/services/` | feature ingress、权限校验、积分、幂等、任务提交等 use case 服务 | 处理 HTTP 细节、返回 FastAPI 响应对象 |
...
2. Feature 执行入口统一走 `FeatureIngressService` -> `FeatureSubmissionService` + `TaskService`（支持 launch/resume）。
