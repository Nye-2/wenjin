...
新体系会扩大数据模型：Prism universal documents、review item v2、sandbox artifacts、provenance links、capability mission schema、room commits 都会同时出现。现有模型经过多轮迁移后已经出现多处重叠：execution / task / run history 重叠，library item / workspace reference 重叠，Prism review / ResultCard review 重叠，LaTeX project / workspace Prism surface 重叠。

DataService 的产品目的不是“再包一层 service”，也不是给旧模型补 facade，而是趁 Super Agent Harness 切换窗口重新定义数据层 SSOT：

- 一个概念只有一个 canonical table family。
- 数据表和领域模型有明确归属，不再按历史功能碎片扩表。
- 所有业务数据库读写通过 repository / domain service 进入。
...
- 迁移完成后 `backend/src/database` 只保留 Base/session/engine/Alembic bootstrap，业务模型归 `backend/src/dataservice/models`。
...
2. 提供 `DataServiceUnitOfWork`，统一 transaction boundary，禁止 repository 自行 commit。
...
`backend/src/database/models` 在迁移完成后不再保留业务模型。新的业务表直接放入 `backend/src/dataservice/models`。旧模型按领域切片迁移；迁移某个领域时，同一提交内更新 imports 和 tests，不保留运行时双模型。
...
| --- | --- | --- |
| Workspace Core | `Workspace`, `WorkspaceSettings`, `Thread` | `workspaces.config`, `workspace_settings.metadata_json`, duplicate workspace/thread pointers |
...
- 新增 architecture tests，锁定“新数据库访问必须走 DataService”的规则，并用 shrinking allowlist 管理旧代码。
- 新增 DataService planning doc，明确模型搬迁顺序、删除顺序、cutover gate。
- 暂不搬旧模型，直到 SSOT ERD 和迁移 map review 通过。

第二阶段开始实现时，第一批真正落地的代码应是：

- `dataservice/unit_of_work.py`
- `dataservice/contracts/common.py`
...
2. 再实现 schema v2、ReviewItem v2、Prism universal document、source/assets/sandbox/provenance 等 canonical table family。
