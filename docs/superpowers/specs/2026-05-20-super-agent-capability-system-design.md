...
6. **数据契约可以做大**：ReviewBatch / ReviewItem、SandboxArtifact、Provenance、PrismChange、CapabilityMission 等契约可以扩展到足够表达长期架构，不为短期省字段牺牲清晰度。
7. **DataService 是 SSOT 数据层重设计**：趁 Super Agent Harness 切换窗口重新收敛数据模型，新增 `dataservice` 模块统一业务模型、repositories、unit-of-work、review state、Prism document data、sandbox artifacts、provenance、room commit 和 projections；迁移完成后 `backend/src/database` 只保留基础设施。
...
| `review_policy` | 产物是否默认进入 ReviewBatch |
...
### 5.3 ReviewBatch v2 Contract

所有可回写结果先进入 review batch；batch 是一次可审阅交付的聚合根，item 是具体目标写入：

```yaml
schema_version: review_batch.v2
id: review_batch_id
workspace_id: workspace_id
execution_id: execution_id
capability_id: idea_to_thesis_manuscript
status: pending          # pending | partially_applied | applied | rejected | failed
title: Results package
summary: 更新结果章节、图表和相关待办
default_selection:
...
items:
  - id: review_item_id
    status: pending        # pending | accepted | rejected | applied | reverted | failed
    target:
      kind: prism_file_change     # prism_file_change | sandbox_artifact | room_document | room_source | room_decision | room_memory | room_task | workspace_asset
      surface: prism
...
actions:
  preview_endpoint: /internal/v1/review/batches/{id}
  apply_endpoint: /internal/v1/review/batches/{id}/apply
  reject_endpoint: /internal/v1/review/batches/{id}/reject
  revert_endpoint: /internal/v1/review/batches/{id}/revert
```
...
Artifact 不应自动进入 Documents。它先进入 review batch/item，用户接受后再 commit。
...
新体系会扩大数据模型：Prism universal documents、review batch/item v2、sandbox artifacts、provenance links、capability mission schema、room commits 都会同时出现。现有模型经过多轮迁移后已经出现多处重叠：execution / task / run history 重叠，library item / workspace reference 重叠，Prism review / ResultCard review 重叠，LaTeX project / workspace Prism surface 重叠。

DataService 的产品目的不是“再包一层 repository facade”，也不是给旧模型补 facade，而是趁 Super Agent Harness 切换窗口重新定义数据层 SSOT，并把数据层升级为 monorepo 内的独立内部服务：

- 一个概念只有一个 canonical table family。
- 数据表和领域模型有明确归属，不再按历史功能碎片扩表。
- 所有 canonical 业务数据库读写只发生在 DataService 服务进程内，通过 repository / domain service 进入。
- Agent、router、Compute、Prism、commit flow 不再到处 import ORM model 或直接 `session.execute`；它们通过 `dataservice_client` 调内部 API。
- Workspace-scoped 数据访问默认带 `workspace_id` / `user_id` 边界。
- Review / apply / provenance / artifact materialization 使用同一套状态机和审计规则。
- 迁移完成后 `backend/src/database` 只保留 Base/session/engine/Alembic bootstrap，业务模型归 `backend/src/dataservice/domains/*/models.py`，并只由 `backend/src/dataservice_app` 服务进程使用。
...
backend/src/dataservice/
  ...
  domains/
    ...
    sandbox/{models.py,contracts.py,repository.py,service.py,projection.py,policy.py}
...
1. 提供 canonical data model，重新收敛 workspace / catalog / execution / review / Prism / source / asset / sandbox / provenance / rooms。
2. 作为独立内部服务部署，Compose 内服务名为 `dataservice`，默认内部地址 `http://dataservice:8080`。
3. 提供 `DataServiceUnitOfWork`，统一 transaction boundary，禁止 repository 自行 commit。
4. 提供 workspace-scoped repositories，统一读写 Library/Source、Documents/Assets、Decisions、Memory、Tasks、Run History projection、Prism、Sandbox。
5. 提供 Prism universal document repository，不再让业务直接依赖 `LatexProject`；新增 `PrismProject` / `PrismDocument` / `PrismFile` / `PrismFileVersion` 作为主模型，现有 workspace-owned LaTeX 项目一次性迁移进去。
6. 提供 ReviewBatch aggregate 和 state machine，使 Prism changes、room outputs、sandbox artifacts 共用一个 review/apply 状态源。
7. 提供 Sandbox runtime repository，管理 workspace sandbox environment、job reproducibility、artifact metadata、input hash、script hash、output path 和 preview metadata。
8. 提供 ProvenanceService，统一 source links、artifact links、execution links、document section links。
9. 提供 projection builders，给 Chat Agent、Lead Agent、Prism context rail、Compute panel 读取轻量上下文。

已接受的 DataService 架构决策：

1. 使用 maintenance-window cutover；迁移后 runtime 直接读 canonical tables，不做 dual-write。
2. 旧表迁移后 rename 为 `_legacy_20260520`，运行时不读，备份验证后 drop。
3. DataService 最终覆盖所有业务模型；第一阶段优先 workspace / Super Agent Harness 数据。
4. DB 只存 metadata 和小文本，大文件由 `workspace_assets` 管元数据并落外部存储。
5. `executions` / `execution_nodes` / `execution_events` 是产品运行状态 SSOT，`task_records` 只作为 worker queue infrastructure。
6. 产品和代码概念统一叫 `Source`，不保留 LibraryItem / Reference 双概念。
...
| `backend/src/dataservice/domains/*/models.py` | yes | no direct business logic | SQLAlchemy table definitions for one aggregate |
| `backend/src/dataservice/domains/*/repository.py` | yes | yes | query/write one aggregate boundary |
| `backend/src/dataservice/domains/*/service.py` | no direct table queries outside repositories | through repositories only | workspace data use cases and state machines |
| `backend/src/dataservice/domains/*/projection.py` | through repositories only | through repositories only | read models for DataService API responses |
| `backend/src/dataservice/domains/operations` | yes | yes | idempotency, outbox, migration reports |
| `backend/src/dataservice_app` | via domain services/repositories | through DataService only | internal FastAPI service process |
| `backend/src/dataservice_client` | no | no | typed HTTP client for other processes |
| routers / agents / compute / execution | no | no | call `dataservice_client` |

`backend/src/database/models` 在迁移完成后不再保留业务模型。新的业务表按聚合归入 `backend/src/dataservice/domains/<domain>/models.py`。旧模型按领域切片迁移；迁移某个领域时，同一提交内更新 imports 和 tests，不保留运行时双模型。

### 11.2 SSOT Domains

第一轮 DataService 需要先定义完整 SSOT，再按领域切片实现：

| Domain | Canonical Models | Redundant Legacy Sources To Converge |
| --- | --- | --- |
| Workspace Core | `Workspace`, `WorkspaceMembership`, `WorkspaceSettings`, `Thread` | `workspaces.user_id` access ownership, `workspaces.config`, `workspace_settings.metadata_json`, duplicate workspace/thread pointers |
| Capability Catalog | `CapabilityDefinition`, `CapabilitySkill`, `CapabilitySeedRevision` | v1 capability runtime fields, skill `config` |
...
| Review | `ReviewBatch`, `ReviewItem`, `ReviewActionLog` | `PrismReviewItem`, `TaskReport.outputs`, ResultCard transient state |
...
2. **Domain service**：表达业务动作，例如 `apply_review_batch`、`create_prism_document_change`、`record_sandbox_artifact`。
...
- Sandbox runner 直接写 Documents；必须先写 SandboxArtifact + ReviewBatch/ReviewItem。
- Queue/task infrastructure 成为用户可见产品状态。
...
2. `ReviewBatch` apply 必须在同一事务内完成目标写入、item 状态变更、batch 状态变更、action log 和 provenance links。
3. `RunHistory` / activity / audit event 可以在事务后发布，但必须引用已提交的 state transition id。
4. Sandbox artifacts 默认不可直接 materialize 到 Documents；必须由 `ReviewBatchService.apply()` 驱动。
5. Prism primary document 只接受 review batch apply 或用户编辑写入，不接受 agent 直接覆盖。
6. Repositories 不允许 commit；完整 use case 由 `DataServiceUnitOfWork` 控制。

### 11.5 SSOT Migration Strategy

现有代码迁移采用“先设计完整 SSOT，再按领域切片直接收敛”，不做长期兼容层：

1. **Final SSOT design gate**：先产出 ERD、表归属、legacy-to-canonical 迁移表、删除列表。
2. **Foundation**：新增 `dataservice` 包、`dataservice_app` 独立服务、`dataservice_client`、`DataServiceUnitOfWork`、architecture guard、Alembic metadata import、Docker/Compose service。
3. **Review first**：用 `ReviewBatch` / `ReviewItem` 统一 `prism_review_items` 与 `TaskReport.outputs` 的可审阅产物。
...
第一阶段只准备 SSOT 设计和 DataService 开发边界，不直接改用户链路：

- 新增 DataService SSOT model spec。
- 新增 `backend/src/dataservice/` 包结构。
- 新增 `backend/src/dataservice_app/` 独立内部服务。
- 新增 `backend/src/dataservice_client/` typed client。
- 新增 `DataServiceUnitOfWork`。
- 新增 DataService Docker target 和 Compose `dataservice` 服务。
- 新增 architecture tests，锁定“新数据库访问必须走 DataService 服务边界”的规则，并用 shrinking allowlist 管理旧代码。
- 新增 DataService planning doc，明确模型搬迁顺序、删除顺序、cutover gate。
- 暂不搬旧模型，直到 SSOT ERD 和迁移 map review 通过。

第二阶段开始实现时，第一批真正落地的代码应是：

- `dataservice/common/unit_of_work.py`
- `dataservice/common/actor.py`
- `dataservice/common/idempotency.py`
- `dataservice/domains/operations/models.py`
- `dataservice/domains/review/models.py`
- `dataservice/domains/review/repository.py`
- `dataservice/domains/review/service.py`
- `dataservice/domains/review/target_handlers.py`
- `dataservice/domains/prism/models.py`
- `dataservice/domains/prism/repository.py`
- `dataservice/domains/prism/service.py`
- `dataservice/domains/source/models.py`
- `dataservice/domains/asset/models.py`
- `dataservice/domains/sandbox/models.py`
- `dataservice/domains/provenance/models.py`
...
1. 先完成 DataService SSOT model spec，明确 ERD、表归属、legacy-to-canonical 迁移表和删除列表。
2. 再实现 schema v2、ReviewBatch / ReviewItem、Prism universal document、source/assets/sandbox/provenance 等 canonical table family。
3. 每个领域切片完成后直接切 runtime consumer 到 DataService，并删除旧 runtime path；旧表只允许离线归档，不作为运行时兼容层。
...
3. 让 sandbox artifacts 进入 review batch/item。
...
1. 新增 schema v2：
   ...
   - ReviewBatch v2: `producer` / `items` / `target` / `payload` / `provenance` / `state` / `actions` / `validation`
...
3. 实现 sandbox runner 最小闭环：
   ...
   - sandbox artifacts 进入 review batch/item，不直接写 Documents。
...
6. DataService 开发先写 SSOT model spec 和 ERD，再做 foundation guard / UnitOfWork；任何新业务 DB 访问都必须经 DataService。
...
10. DataService 作为 SSOT 数据层重设计模块，承接业务模型、Prism、review、sandbox artifact、provenance、room commit、execution projection 的统一访问。
