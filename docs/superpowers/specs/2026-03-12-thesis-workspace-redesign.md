...
- `task_type="workspace_feature"` → `execute_workspace_feature()`
- `task_type="deep_research"` → `SkillTaskHandler.execute_skill()`（已有路径）
- `task_type="thesis_generation"` → 经由 `execute_workspace_feature()` → `_is_thesis_payload()` → `execute_thesis_generation()`
