...
| 模块 | 字段 | 数据来源 |
|------|------|---------|
| deep_research | `status` | 查 `tasks` 表最近一次 `task_type="deep_research"` 的 status |
| deep_research | `ideas_count` | 查 `artifacts` 表 `type="research_ideas"` 的数量 |
| deep_research | `papers_count` | 查最近 DR task result 中的 papers 数组长度 |
| literature | `total` / `core` | 查 `workspace_literature` 表（独立表，非 artifact） |
| opening_research | `status` | 查 `artifacts` 表中 `type in ("opening_report", "literature_review", "feasibility_analysis")` 是否存在 |
| thesis_writing | `outline_done` | 查 `artifacts` 表 `type="framework_outline"` 是否存在 |
| thesis_writing | `chapters_done` / `chapters_total` | 查 `artifacts` 表 `type="thesis_chapter"` 数量 / 大纲 artifact 中的总章节数 |
| figure_generation | `count` | 查 `artifacts` 表 `type="figure"` 的数量 |
| compile_export | `status` | 查 `artifacts` 表 `type="paper_draft"` 是否存在 |
```
...
Deep Research 在 registry 中注册（`task_type="deep_research"`），但执行时仍走现有的 `SkillTaskHandler → SkillExecutor → DeepResearchSkillV2` 路径。

**关键**：Deep Research 不经过 `workspace_feature_handler.py` 的 `execute_workspace_feature()` 函数。当前 task 分发是在 `task/registry.py` 层面按 `task_type` 路由的：

- `task_type="workspace_feature"` → `execute_workspace_feature()`（若 `_is_thesis_payload()` 为 True 则转入 `execute_thesis_generation()`）
- `task_type="deep_research"` → `SkillTaskHandler.execute_skill()`（已有路径）
- `task_type="thesis_generation"` → 直接路由到 `execute_thesis_generation()`（不经过 `execute_workspace_feature()`）

Deep Research 的 `task_type="deep_research"` 使其在 task 分发层面就被路由到 `SkillTaskHandler`，完全绕过 `workspace_feature_handler.py`。因此不需要修改 `execute_workspace_feature()` 来处理 Deep Research。

前端通过 `features router` 提交 task 时，`task_type` 来自 registry 定义。`features.py` 中 `submit_task()` 会使用 feature 的 `task_type` 字段，确保 Deep Research 走正确的执行路径。

这样 Deep Research 既能通过 `/features` 接口被前端发现（registry 注册），又能复用已有的 skill 执行基础设施，且不需要修改 `workspace_feature_handler.py` 的任何逻辑。
...
### 11.4 _is_thesis_payload 和常量集合更新

现有的 `_is_thesis_payload()` 函数检查 `workspace_type`、`agent`、`handler_key` 三个维度。handler_key 变更后，`THESIS_HANDLER_KEYS` 常量必须同步更新：
...
注意以下两个 handler_key 不在此集合中：
- `thesis.deep_research`：`task_type="deep_research"`，在 task 分发层面就路由到 `SkillTaskHandler`，不经过 `execute_workspace_feature()`
- `thesis.literature_management`：`task_type="workspace_feature"` 且 `panel=None`，走通用 handler 路径而非 thesis workflow

同时 `THESIS_AGENTS` 集合也需要审查——新增的 `deep_research` agent 和 `scout` agent 不应触发 thesis workflow：

```python
# 只保留真正需要走 thesis workflow 的 agent
THESIS_AGENTS = {"thesis_writer"}
```

`execute_thesis_generation()` 内部增加 `action` 参数分流，对其余现有代码的侵入最小。
...
GAP_ANALYSIS = "gap_analysis"          # Deep Research 已在使用但未在枚举中定义
```

注意：`gap_analysis` 类型已被 `DeepResearchSkillV2._create_artifacts()` 使用（deep_research.py 第 969 行），但未在 `ArtifactType` 枚举中定义。应趁此次改动一并补上。
...
`literature_management` feature 的 `panel=None` 意味着它是纯管理型模块，不提交异步 task。前端对 `panel=None` 的 feature 应区别处理：
- 卡片点击后直接导航到 `/workspaces/[id]/literature` 管理页面
- 不触发 `/features/{feature_id}/execute` 接口调用
- 不进入 task 轮询流程
- `ModuleCard` 组件根据 `panel` 是否为 null 决定渲染"管理"按钮还是"开始"按钮
