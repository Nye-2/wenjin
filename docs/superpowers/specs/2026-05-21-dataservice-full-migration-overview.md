...
- Immutable audit events are now owned behind `AuditDataService`. `AuditService` remains a best-effort facade with test model injection, but no longer imports or queries `AuditLog` directly. The architecture guard now treats `AuditLog` as migrated for runtime imports.
