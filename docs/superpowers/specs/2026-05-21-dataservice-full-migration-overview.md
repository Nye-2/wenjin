...
- Account business reads and writes now have an `AccountDataService` boundary. `UserService`, user dashboard profile/credit projection, admin dashboard user counts/list/status/role mutation, admin analytics user-growth/adoption counts, and refresh-token lookup helper no longer query `User` directly from service code. Gateway auth dependencies still return the concrete authenticated user object as an auth-boundary subject until the final account client/projection cutover.
