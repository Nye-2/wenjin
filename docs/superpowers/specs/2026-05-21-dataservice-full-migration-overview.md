...
- Cross-workspace user knowledge memory is now owned behind `KnowledgeDataService`. `KnowledgeService` is a compatibility facade; memory injection, memory extraction, compaction, and upload-context memory writes no longer import `UserKnowledge` or `KnowledgeCategory` runtime ORM symbols. The architecture guard now treats `UserKnowledge` / `KnowledgeCategory` as migrated for runtime imports.
