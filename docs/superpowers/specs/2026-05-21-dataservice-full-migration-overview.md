...
- Credit grant-rule CRUD, active rule lookup, registration-bonus application, and periodic grant execution are now owned behind `CreditDataService`. `CreditGrantRuleService` keeps config validation and admin-log orchestration, while the Celery periodic task delegates rule lookup, target filtering, user balance mutation, transaction creation, and `last_triggered_at` updates to DataService. The architecture guard now treats `CreditGrantRule` as migrated for runtime imports.
