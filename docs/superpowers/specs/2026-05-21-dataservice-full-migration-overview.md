...
- `backend/src/dataservice/domains/rooms/` owns room contracts, repository, projections, service, and review handler factory. Workspace room HTTP routes still expose product-specific endpoint adapters, but canonical decisions/memory/task business logic lives in `RoomsDataService`.
