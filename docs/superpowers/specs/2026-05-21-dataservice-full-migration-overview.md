...
- `backend/src/dataservice/domains/rooms/` owns room contracts, repository, projections, service, and review handler factory. Workspace room HTTP routes for decisions, memory, and workspace tasks now call `RoomsDataService` directly instead of old service facades.
...
- The former room service facades have been removed; workspace room endpoints now call DataService APIs directly for decisions, memory, workspace tasks, settings, sandbox environment-touch, library, and documents.
...
- Workspace Prism context now reads decision/memory context through `RoomsDataService`; the workspace sandbox exec endpoint calls `SandboxDataService` directly for environment state.
- Workspace library room endpoints call `SourceDataService` directly, so library room writes no longer create `library_items` rows.
- Workspace documents room endpoints call `AssetDataService` directly for document create/read/update/delete/version operations, so document room writes now create canonical `workspace_assets` rows and no longer create `documents_v2` rows.
...
- Workspace settings room endpoints call `WorkspaceDataService` directly, so `workspace_settings` is now owned by the Workspace aggregate instead of a standalone room service.
...
- `ThreadService` now acts as a `ConversationDataService` facade for thread create/get/list/update/delete and no longer imports or queries the `Thread` ORM directly. The architecture guard now treats `Thread` as migrated for runtime imports, allowing only DataService/database ownership packages and `TYPE_CHECKING` annotations.
...
- `WorkspacePrismService` no longer directly imports or queries the `LatexProject` ORM. It resolves LaTeX adapter projects through `WorkspaceLatexProjectService`, while `LatexProjectService` / `WorkspaceLatexProjectService` remain the explicit adapter owner until the later Prism universal-document adapter deletion slice.
...
- Workspace Activity thread cards now read thread summaries through `ConversationDataService.list_workspace_thread_summaries()` plus canonical message projections; the activity aggregator no longer directly queries or imports the `Thread` ORM for thread feed entries.
...
- Provenance link create/list/delete now has internal DataService routes matching the existing typed client contracts, so Source/Prism/Review traceability can move across the standalone DataService boundary without in-process-only calls.
...
- Workspace metadata helpers, dashboard workspace-type resolution, and user dashboard workspace stats now read through `WorkspaceDataService`; `WorkspaceStatsRecord` is the dashboard-facing projection for total/by-type/recent workspace counts. `feature_action_resolution_service` keeps `Workspace` / `Artifact` only as type-checking annotations, with no runtime ORM import.
- Gateway workspace access helpers, workspace runtime helpers, workspace serializers, and the academic workspace facade no longer import the `Workspace` ORM model at runtime; serializers now accept the DataService `WorkspaceRecord` projection shape for the next router cutover.
- Artifact version allocation now locks workspace rows through `WorkspaceDataService.lock_workspace_for_update()`, so the legacy `ArtifactService` no longer imports `Workspace` at runtime while its old artifact version-chain semantics remain isolated for the later asset/artifact cutover.
- Admin workspace adoption/dashboard stats now use `WorkspaceDataService` admin projections and membership-based workspace counts; reference BibTeX sync resolves workspace metadata through `WorkspaceDataService`. The architecture guard now treats `Workspace` as migrated for runtime imports, ignoring only `TYPE_CHECKING` annotations and DataService/database ownership packages.
- Legacy artifact payload CRUD/list/count/lineage is now owned behind `AssetDataService` as `LegacyArtifactProjection` / command contracts while the physical `artifacts` table is still converging. The old academic `ArtifactService` is now a DataService facade; dashboard artifact status/recent-artifact projections, admin artifact totals, and deep-search artifact imports no longer import or query the `Artifact` ORM directly. The architecture guard now treats `Artifact` as migrated for runtime imports, allowing only DataService/database ownership packages and `TYPE_CHECKING` annotations.
- Legacy generation usage CRUD/list/stats/cleanup is now owned behind `ExecutionDataService` as `GenerationRecordProjection` / command contracts while the physical `generation_records` table remains a legacy storage table converging into execution/assets. The old academic `GenerationService` is now a DataService facade, and runtime code no longer imports or queries the `GenerationRecord` ORM directly. The architecture guard now treats `GenerationRecord` as migrated for runtime imports.
- Worker task queue persistence is now owned behind `TaskDataService`, internal DataService routes, and typed client contracts as `TaskRecordProjection` / command contracts while `task_records` remains queue infrastructure. `TaskStore` retains Redis runtime cache, workspace event publishing, and execution transition orchestration, but no longer imports or queries the `TaskRecord` ORM directly. The architecture guard now treats `TaskRecord` as migrated for runtime imports.
- Admin audit-log create/list operations are now owned behind `CatalogDataService` as `AdminLogRecord` contracts. Capability/skill admin mutations, admin dashboard log create/list, credit grant-rule mutations, and redeem-code admin actions no longer import or construct `AdminLog` directly. The architecture guard now treats `AdminLog` as migrated for runtime imports.
- Cross-workspace user knowledge memory is now owned behind `KnowledgeDataService`, internal DataService routes, and typed client contracts. `KnowledgeService` is a compatibility facade; memory injection, memory extraction, compaction, and upload-context memory writes no longer import `UserKnowledge` or `KnowledgeCategory` runtime ORM symbols. The architecture guard now treats `UserKnowledge` / `KnowledgeCategory` as migrated for runtime imports.
- Workspace template CRUD and active-template switching are now owned behind `TemplateDataService`, internal DataService routes, and typed client contracts. `TemplateService` keeps parsing and source-file cleanup orchestration, but no longer imports or queries `WorkspaceTemplate` directly. The architecture guard now treats `WorkspaceTemplate` as migrated for runtime imports.
- Immutable audit events are now owned behind `AuditDataService`, internal DataService routes, and typed client contracts. `AuditService` remains a best-effort facade with test model injection, but no longer imports or queries `AuditLog` directly. The architecture guard now treats `AuditLog` as migrated for runtime imports.
- LaTeX adapter project metadata, template catalog seeding/listing, and compile-history persistence are now owned behind `LatexDataService`, internal DataService routes, and typed client contracts. `LatexProjectService`, `LatexTemplateService`, `LatexCompileService`, `WorkspaceLatexProjectService`, and `WorkspacePrismService` remain orchestration/file-system facades, but no longer import or query `LatexProject`, `LatexTemplate`, or `LatexCompileHistory` directly. Workspace bridge lookup is exposed as a DataService-owned minimal projection, and the architecture guard now treats all three LaTeX ORM models as migrated for runtime imports.
- Credit grant-rule CRUD, active rule lookup, registration-bonus application, periodic grant execution, balance/history reads, token-consumption accounting, consumption/refund writes, admin grant/deduct writes, redeem-code lifecycle, referral relationship writes, referral bonus triggers, and admin credit analytics/dashboard projections are now owned behind `CreditDataService`, internal DataService routes, and typed client contracts. `CreditGrantRuleService`, `CreditService`, `CreditRedeemService`, and `ReferralService` remain orchestration/validation facades; runtime code no longer imports or queries `CreditTransaction`, `CreditRedeemCode`, `CreditRedemption`, `Referral`, or `CreditGrantRule` directly. The architecture guard now treats those credit/referral ORM models as migrated for runtime imports.
- Account business reads and writes now have an `AccountDataService` boundary plus internal DataService routes and typed client contracts. `UserService`, user dashboard profile/credit projection, admin dashboard user counts/list/status/role mutation, admin analytics user-growth/adoption counts, and refresh-token lookup helper no longer query `User` directly from service code. Gateway auth dependencies still return the concrete authenticated user object as an auth-boundary subject until the final account client/projection cutover.
- DataService internal HTTP and typed-client exposure now covers every in-process DataService API surface. The app/client gap check is empty for `account`, `asset`, `audit`, `catalog`, `conversation`, `credit`, `execution`, `knowledge`, `latex`, `prism`, `prism_review`, `provenance`, `review`, `rooms`, `sandbox`, `source`, `task`, `template`, and `workspace`.
...
- Verification through the Source curation/evidence/indexer/asset/upload-preprocess/BibTeX snapshot cleanup, workspace-run table drop, run-history route cutover, room direct-DataService route cutover, Prism action-contract cleanup, conversation JSON-write removal, ThreadService facade cutover, workspace activity thread-summary DataService cutover, Workspace Prism adapter boundary cleanup, owner invariant, review transaction cleanup, workspace metadata/dashboard projection, gateway workspace helper/serializer, artifact payload DataService, generation usage DataService, task persistence service/client DataService, admin audit-log DataService, knowledge/memory service/client DataService, workspace template service/client DataService, audit event service/client DataService, LaTeX adapter service/client DataService, credit/referral service/client DataService, account service/client DataService, provenance route/client DataService, and Workspace/Artifact/Thread/GenerationRecord/TaskRecord/AdminLog/UserKnowledge/WorkspaceTemplate/AuditLog/LatexProject/LatexTemplate/LatexCompileHistory/CreditGrantRule/CreditTransaction/CreditTransactionType/CreditRedeemCode/CreditRedemption/Referral runtime-import guard slices is green through `cd backend && .venv/bin/python -m pytest tests/ -q` with 1952 backend tests; ThreadService facade/conversation target tests pass with 30 tests; Workspace Prism adapter boundary target tests pass with 22 tests; task persistence service/client DataService target tests pass with 118 tests; admin audit-log DataService target tests pass with 25 tests; credit/admin-log integration target tests pass with 41 tests; credit/referral service/client DataService target tests pass with 41 tests; account service DataService target tests pass with 53 tests; all DataService app/client foundation target tests pass with 20 tests; DataService app/client closure target tests pass with 116 tests; knowledge/memory service/client DataService target tests pass with 60 tests; memory/upload integration target tests pass with 89 tests; workspace template DataService target tests pass with 22 tests; audit event DataService target tests pass with 17 tests; LaTeX adapter service/client DataService target tests pass with 82 tests; workspace activity thread-summary DataService target tests pass with 26 tests; workspace metadata/dashboard DataService projection target tests pass with 81 tests; gateway workspace helper/serializer target tests pass with 72 tests; artifact payload DataService target tests pass with 56 tests; generation usage DataService target tests pass with 16 tests; Workspace/Artifact/Thread/GenerationRecord/TaskRecord/AdminLog/UserKnowledge/WorkspaceTemplate/AuditLog/LatexProject/LatexTemplate/LatexCompileHistory/CreditGrantRule/CreditTransaction/CreditTransactionType/CreditRedeemCode/CreditRedemption/Referral runtime-import architecture guard passes with 5 tests; owner invariant target tests pass with 11 tests; review transaction target tests pass with 14 tests; execution commit DataService target tests pass with 34 tests; execution engine run-history event tests pass with 16 tests; workspace run-history route cutover target tests pass with 28 tests; room direct-DataService route target tests pass with 34 tests; sandbox route cutover target tests pass with 32 tests; library route cutover target tests pass with 44 tests; documents route cutover target tests pass with 34 tests; `cd frontend && npm run typecheck` and `cd frontend && npm run lint` also pass.
