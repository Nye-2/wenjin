...
- Workspace template CRUD and active-template switching are now owned behind `TemplateDataService`. `TemplateService` keeps parsing and source-file cleanup orchestration, but no longer imports or queries `WorkspaceTemplate` directly. The architecture guard now treats `WorkspaceTemplate` as migrated for runtime imports.
