...
- Following DeerFlow's runtime middleware pattern, ReactSubagent harness tool exceptions are downgraded at the adapter boundary into bounded JSON error results instead of aborting the subagent turn. The adapter still records `status=failed`, publishes `execution.harness.tool_call.failed`, and preserves `recoverable_error` / `error_code` metadata; LangGraph control-flow exceptions and loop guard hard stops remain hard stops.
...
- Harness `sandbox.run_python` now returns `execution_manifest(schema=wenjin.harness.run_python.execution_manifest.v1)` on every result, carrying workspace/execution/node/invocation identity, safe script path, dependency hints, sandbox job/environment ids, network profile, and effective timeout. User-code nonzero exits are downgraded into recoverable tool results that preserve the runner payload and add `failure_classification(schema=wenjin.harness.run_python.failure_classification.v1)` plus `error_code=python_exit_nonzero`; the LangChain adapter projects both records into tool record metadata and `execution.harness.tool_call.completed` metadata so later replanning and run-detail surfaces do not parse raw tool JSON.
