...
- `sandbox.run_python` returns `execution_manifest` for run identity and `reproducibility_manifest` for experiment evidence. The latter is intentionally summary-shaped: script path, requested/installed dependencies, sandbox job ids, generated artifact paths, and command risk levels. It must never include raw stdout/stderr, raw command argv, environment variables, secrets, host paths, or protected/internal refs.
