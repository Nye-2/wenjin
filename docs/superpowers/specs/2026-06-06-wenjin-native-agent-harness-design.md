...
- Following DeerFlow's runtime middleware pattern, ReactSubagent harness tool exceptions are downgraded at the adapter boundary into bounded JSON error results instead of aborting the subagent turn. The adapter still records `status=failed`, publishes `execution.harness.tool_call.failed`, and preserves `recoverable_error` / `error_code` metadata; LangGraph control-flow exceptions and loop guard hard stops remain hard stops. Tool input schema validation failures are also recoverable: LangChain/Pydantic errors return `error_code=tool_input_validation`, write a failed tool record with field/type/message summary, and do not echo raw invalid payloads back into debug records.
...
- Tool input schema validation errors must be handled at the adapter boundary, because they happen before the tool coroutine enters `_invoke_recorded()`. They should return the same JSON envelope shape as other recoverable harness errors, include only safe validation fields (`loc`, `msg`, `type`, count), and record a failed `_harness_tool_records` entry without raw invalid args.
