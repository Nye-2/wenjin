...
- `reproducibility_manifest.sandbox.retry_count` now records the missing-dependency retry count, and recoverable user-code failures get a bounded `report_markdown` Recovery guidance section. This gives the next team member enough action context to revise code or dependency hints inside the same workspace sandbox without parsing raw stderr.
- Harness context assembly now exposes a stable team-member execution package at `wenjin.harness.context_bundle.v1`: `capability_goal`, `member_role`, `allowed_tools`, `workspace_roots`, `search_ignored_names`, latest file-change/sandbox-execution/reproducibility summaries, replan signals, and upstream artifact candidates are top-level bounded fields. ReactSubagent passes its resolved tool names into this bundle, so workflow/prompt quality tuning can depend on schema fields instead of ad hoc prompt text.
...
- `/workspace/reports/artifacts.json(schema=wenjin.workspace_sandbox.artifact_manifest.v1)` is an editable artifact metadata manifest. Layout initialization creates it only when missing and never overwrites an existing manifest. It records user-reviewable generated artifacts under `/workspace/outputs/**` and `/workspace/reports/**`; entries should prefer title, description, artifact_kind, content_hash, source_script, dataset_paths, and review notes when known.
...
- `sandbox.register_dataset` is the only model-facing dataset-provenance write helper. It requires both `filesystem.write` and `filesystem.diff`, writes only `/workspace/datasets/manifest.json`, reuses the layout merge/sanitization rules, preserves existing user-authored rows by path, rejects non-dataset/guidance/protected/internal refs, drops host-path or secret-like provenance values, and emits the same harness `file_change` evidence as other write tools.
- `sandbox.register_artifact` is the only model-facing artifact-metadata write helper. It requires both `filesystem.write` and `filesystem.diff`, writes only `/workspace/reports/artifacts.json`, reuses layout merge/sanitization rules, preserves existing user-authored rows by path, rejects non-artifact/guidance/protected/internal refs, drops host-path or secret-like values, and emits the same harness `file_change` evidence as other write tools. Artifact discovery skips this manifest and `.gitkeep` files, then enriches generated artifact candidates from same-path manifest metadata.
