...
- `sandbox.list_dir`, `sandbox.glob`, and `sandbox.grep` filter protected/internal paths; protected/internal files must not be discoverable through search/listing even when direct reads are also blocked.
