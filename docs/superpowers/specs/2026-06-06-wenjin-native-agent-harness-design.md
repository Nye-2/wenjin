...
- Tool output budget now externalizes oversized `sandbox.read_file` output and Lead-owned `sandbox.run_python` stdout/stderr into `/workspace/outputs/harness/{execution_id}/{node_id}/{invocation_id}/`, returning compact previews plus `output_refs`; `sandbox.read_file(max_chars=...)`, `sandbox.glob(max_matches=...)`, and `sandbox.grep(max_matches=...)` clamp caller requests to policy maxima; `sandbox.list_dir` also caps structured entries with total/returned counters, while `sandbox.glob` / `sandbox.grep` cap structured matches with `returned_matches` / `match_limit`. `sandbox.grep` skips oversized files, binary files, and overlong lines using `grep_max_file_bytes` / `grep_max_line_chars`, then reports scanned/skipped counts. ReactSubagent tool records and `execution.harness.output_externalized` events retain refs for externalized outputs.
