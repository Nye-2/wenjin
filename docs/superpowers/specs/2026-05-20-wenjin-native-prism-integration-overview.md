...
Date: 2026-05-20
Status: Product Architecture Overview

## 1. Overview

Workspace Prism surface 已经完成第一阶段收敛：
...
- result card / completed view 的 Prism action 会落到 `/workspaces/:id/prism`
- 数据库通过 partial unique index 保证一个 workspace 只有一个 primary manuscript

这意味着链路已经闭环，但它仍然更像“workspace 挂了一个 manuscript editor”。下一阶段要解决的是产品级适配：
...
`result_card` checkbox、Compute completed view、Prism file-change review 不应继续是三套近似流程。下一阶段要收敛成统一的 review item contract：
...
下一阶段实现必须走 canonical schema 和单一路径：
...
2. “预览待确认修改”进入 `/workspaces/:id/prism?focus=file_changes`。
...
下一阶段先建立 canonical persistent contracts，再扩展 product projection。新增产品语义不继续写进 `LatexProject.llm_config.metadata`：
...
稿件 provenance 不应塞进正文注释或 `LatexProject.llm_config.metadata` 里作为事实源。下一阶段直接使用 `prism_source_links`：
...
| References | `ReferenceBibTeXService.sync_prism` | 同步引用时生成 source links / citation trace |
...
| TaskReport | 写作类 output 带 `review_target=prism_file_change` 和 provenance |
| Lead runtime | 在 pending changes 未处理时提示用户确认或先合并，不盲目追加 |
...
- TaskReport writing output 附 source usage
- Prism pending change 保存 canonical source links
...
把本 overview 拆成 implementation plan：

```text
Plan: 2026-05-20-wenjin-native-prism-integration.md

Task 1. Extend Prism surface projection with workspace context
Task 2. Add canonical Prism review/provenance/protected-section schema and one-time migration
Task 3. Introduce DB-backed ResultReviewItem projection and actions
Task 4. Build Prism context rail and unified review renderer
Task 5. Persist provenance links from TaskReport writing outputs
Task 6. Feed protected sections and lightweight manuscript state into launch_feature context
```

这五步能把 Prism 从“已绑定的 manuscript surface”推进到“Wenjin-native manuscript collaboration surface”。
