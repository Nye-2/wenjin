...
- Test: `backend/tests/task/test_thesis_registry.py`

注意：`backend/tests/workspace_features/` 目录不存在，registry 测试放在 `tests/task/` 中（与现有的 `test_workspace_feature_handler.py` 同级）。

- [ ] **Step 1: 写测试 — 验证新 registry 定义**

```python
# backend/tests/task/test_thesis_registry.py
...
Run: `cd /home/cjz/AcademiaGPT-V2/backend && uv run pytest tests/task/test_thesis_registry.py::TestThesisRegistryUpdate -v`
Expected: FAIL
...
`tests/gateway/routers/test_features.py` 中 `test_get_workspace_features_returns_registry_features` 断言了旧的 thesis feature ID 列表，需要更新：

```python
# tests/gateway/routers/test_features.py
# 在 test_get_workspace_features_returns_registry_features 中，替换:
...
`tests/task/test_workspace_feature_handler.py` 中现有的 `test_copyright_materials_handler_persists_artifact` 不涉及 thesis feature，无需修改。

- [ ] **Step 7: 提交**

```bash
cd /home/cjz/AcademiaGPT-V2 && git add backend/src/workspace_features/registry.py backend/tests/task/test_thesis_registry.py backend/tests/gateway/routers/test_features.py && git commit -m "feat: replace THESIS_FEATURES with new 6-module design"
```
...
**设计说明：** 新的 6 模块设计改变了路由逻辑。分析 `_dispatch_task()` 路由规则：
- `task_type="thesis_generation"` → 直接到 `execute_thesis_generation()`，不经过 `execute_workspace_feature()`
- `task_type="workspace_feature"` → `execute_workspace_feature()` → `_is_thesis_payload()` 检查
- `task_type="deep_research"` → SkillTaskHandler

关键问题：`compile_export` 的 agent 是 `thesis_writer`，而 `thesis_writer` 在 `THESIS_AGENTS` 中。如果 `_is_thesis_payload()` 继续使用 OR 逻辑检查 agent，`compile_export` 会被误路由到 thesis LangGraph workflow。

解决方案：**废弃旧的三条件 OR 逻辑**。既然所有需要走 thesis workflow 的 feature 都使用 `task_type="thesis_generation"`（在 `_dispatch_task` 层面直接路由），`_is_thesis_payload()` 在 `execute_workspace_feature()` 内的检查已经是死代码路径——任何到达 `execute_workspace_feature()` 的 payload 必然是 `task_type="workspace_feature"`，应该走通用 handler。因此 `_is_thesis_payload()` 应该始终返回 False，或者直接移除这个分支。

实现策略：
- `THESIS_WORKSPACE_TYPES` = 空集
- `THESIS_AGENTS` = 空集
- `THESIS_HANDLER_KEYS` = 空集
- `_is_thesis_payload()` 始终返回 False
- 保留函数签名以避免导入断裂

- [ ] **Step 1: 写测试 — 验证常量更新和路由行为**
...
Expected: FAIL — `THESIS_WORKSPACE_TYPES` 不为空，`THESIS_HANDLER_KEYS` 仍是旧值
...
使用 Alembic autogenerate 创建迁移，不要手写 revision ID（手写容易与其他分支冲突）。

```bash
cd /home/cjz/AcademiaGPT-V2/backend && uv run alembic revision --autogenerate -m "add workspace_literature table"
```

如果 autogenerate 不可用（例如无数据库连接），手动创建迁移文件。注意：先查看 `backend/alembic/versions/` 中最新的 revision ID 作为 `down_revision`，当前最新是 `003`：
...
def create_test_app(user, workspace_service, dashboard_service):
    from src.gateway.routers.workspaces import router, get_workspace_service
...
"""Tests for thesis workspace feature handlers (figure_generation, compile_export)."""
...
Run: `cd /home/cjz/AcademiaGPT-V2/backend && uv run pytest tests/task/test_thesis_handlers.py -v`
Expected: FAIL — handler not registered

- [ ] **Step 3: 实现 thesis.py handlers**
...
from src.workspace_features.runtime import register_feature_handler
...
# 添加:
from src.workspace_features.handlers import thesis  # noqa: F401
```
...
```bash
cd /home/cjz/AcademiaGPT-V2 && git add backend/src/workspace_features/handlers/thesis.py backend/src/workspace_features/handlers/__init__.py backend/tests/task/test_thesis_handlers.py && git commit -m "feat: add figure_generation and compile_export handlers"
```

---

### Task 8: Thesis workflow action 路由（thesis_writing, opening_research）
...
**设计说明：** 当前 `ExecuteResponse` 模型要求 `task_id: str`（非 nullable），不支持不提交 task 直接返回 warning 的场景。需要修改 response model 支持 `task_id: str | None` 和新增 `warning` / `detail` 字段。同时 `execute_feature` 需要注入 `LiteratureService` 依赖以查询文献数量。

- [ ] **Step 1: 写测试 — ExecuteResponse 支持 warning 和 nullable task_id**
...
Run: `cd /home/cjz/AcademiaGPT-V2/backend && uv run pytest tests/gateway/routers/test_features.py::TestExecuteResponseModel tests/gateway/routers/test_features.py::TestLiteratureInsufficientWarning -v`
Expected: FAIL

- [ ] **Step 4: 实现 — 修改 ExecuteResponse 模型**
...
- [ ] **Step 5: 实现 — 在 execute_feature 中添加文献检测**

在 `features.py` 中：
1. 添加 `LiteratureService` 依赖注入（`from src.services.literature_service import LiteratureService, get_literature_service`）
2. 在 `execute_feature` 签名中添加 `literature_service: LiteratureService = Depends(get_literature_service)`
3. 当 `feature_id == "thesis_writing"` 时，检查 `request.params` 中的 action，如果 `action in ("write_chapter", "write_all")`，调用 `literature_service.count_literature(workspace_id)` 检查文献数量
4. 不足时返回 `ExecuteResponse(task_id=None, status="warning", warning="literature_insufficient", ...)`

```python
LITERATURE_THRESHOLD = 15  # 推荐的最低文献数量

# 在 execute_feature 中，提交 task 之前添加:
if feature_id == "thesis_writing":
    action = request.params.get("action", "write_all")
...
Run: `cd /home/cjz/AcademiaGPT-V2/backend && uv run pytest tests/gateway/routers/test_features.py -v`
Expected: PASS
...
## Chunk 4: 前端模块实现 (Tasks 15-21)

### Task 15: Deep Research 工作区
...
### Task 20: KnowledgePanel 新增 artifact 类型映射
