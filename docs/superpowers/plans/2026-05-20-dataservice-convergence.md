...
- Workspace decisions, memory, tasks, settings, and sandbox environment-touch endpoints now call DataService APIs directly, and their old room facade files have been deleted. Remaining room endpoint adapters are limited to library and documents while their route contracts are still being cut over.
