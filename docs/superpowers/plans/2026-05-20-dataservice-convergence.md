...
- Workspace decisions, memory, and tasks endpoints now call `RoomsDataService` directly, and their old room facade files have been deleted. Remaining room endpoint adapters are limited to library, documents, settings, and sandbox while their route contracts are still being cut over.
