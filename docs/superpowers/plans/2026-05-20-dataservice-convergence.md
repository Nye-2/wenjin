...
- Workspace decisions, memory, tasks, settings, sandbox environment-touch, and library endpoints now call DataService APIs directly, and their old room facade files have been deleted. The remaining room endpoint adapter is documents while that route contract is still being cut over.
