...
- Legacy generation usage CRUD/list/stats/cleanup is now owned behind `ExecutionDataService` as `GenerationRecordProjection` / command contracts while the physical `generation_records` table remains a legacy storage table converging into execution/assets. The old academic `GenerationService` is now a DataService facade, and runtime code no longer imports or queries the `GenerationRecord` ORM directly. The architecture guard now treats `GenerationRecord` as migrated for runtime imports, allowing only DataService/database ownership packages.
- Worker task queue persistence is now owned behind `TaskDataService`, internal DataService routes, and typed client contracts as `TaskRecordProjection` / command contracts while `task_records` remains queue infrastructure. `TaskStore` retains Redis runtime cache, workspace event publishing, and execution transition orchestration, but no longer imports or queries the `TaskRecord` ORM directly. The architecture guard now treats `TaskRecord` as migrated for runtime imports, allowing only DataService/database ownership packages.
- Admin audit-log create/list operations are now owned behind `CatalogDataService` as `AdminLogRecord` contracts. Capability/skill admin mutations, admin dashboard log create/list, credit grant-rule mutations, and redeem-code admin actions no longer import or construct `AdminLog` directly. The architecture guard now treats `AdminLog` as migrated for runtime imports, allowing only DataService/database ownership packages.
- Cross-workspace user knowledge memory is now owned behind `KnowledgeDataService`, internal DataService routes, and typed client contracts. `KnowledgeService` is a compatibility facade; memory injection, memory extraction, compaction, and upload-context memory writes no longer import `UserKnowledge` or `KnowledgeCategory` runtime ORM symbols. The architecture guard now treats `UserKnowledge` / `KnowledgeCategory` as migrated for runtime imports, allowing only DataService/database ownership packages.
- Workspace template CRUD and active-template switching are now owned behind `TemplateDataService`, internal DataService routes, and typed client contracts. `TemplateService` keeps parsing and source-file cleanup orchestration, but no longer imports or queries `WorkspaceTemplate` directly. The architecture guard now treats `WorkspaceTemplate` as migrated for runtime imports, allowing only DataService/database ownership packages.
- Immutable audit events are now owned behind `AuditDataService`, internal DataService routes, and typed client contracts. `AuditService` remains a best-effort facade with test model injection, but no longer imports or queries `AuditLog` directly. The architecture guard now treats `AuditLog` as migrated for runtime imports, allowing only DataService/database ownership packages.
