...
- Current gateway/worker process boundary follow-up
  - Execution commit 的 Library output materialization 不再从 `execution_service.db` 读取 DB session；Prism `refs.bib` sync 直接使用当前 Source/Prism DataService client。
  - Gateway lifespan 移除 DB init/close；readiness 的 persistence dependency 改为 standalone DataService `/readyz`。
  - Worker bootstrap/shutdown 移除 `reset_db_engine`、`init_db`、`close_db`；worker 进程只初始化 Sentry、Redis 和 MCP runtime。
  - Runtime helper TYPE_CHECKING imports 从 DB `Thread` / `Workspace` model 改为 DataService client payload contract。
  - Architecture guard 新增 `test_gateway_process_stays_off_database_lifecycle`、`test_worker_process_stays_off_database_lifecycle` 和 `test_runtime_type_hints_use_dataservice_contracts_not_database_models`。
...
- `cd backend && .venv/bin/python -m pytest tests/architecture/test_dataservice_boundaries.py::test_reference_library_runtime_uses_dataservice_boundary tests/services/test_execution_commit_service.py::test_commit_library_item_syncs_prism_bibliography_without_db_session -q` -> red first: 2 failed; after implementation: passed.
- `cd backend && .venv/bin/python -m pytest tests/architecture/test_dataservice_boundaries.py::test_gateway_process_stays_off_database_lifecycle tests/gateway/test_health.py::test_build_readiness_report_aggregates_component_health tests/gateway/test_health.py::test_check_dataservice_uses_dataservice_readyz -q` -> red first: failed; after implementation: passed.
- `cd backend && .venv/bin/python -m pytest tests/architecture/test_dataservice_boundaries.py::test_worker_process_stays_off_database_lifecycle tests/task/test_worker.py::test_bootstrap_worker_runtime_degrades_on_mcp_validation_errors -q` -> red first: failed; after implementation: passed.
- `cd backend && .venv/bin/python -m pytest tests/architecture/test_dataservice_boundaries.py::test_runtime_type_hints_use_dataservice_contracts_not_database_models -q` -> red first: failed; after implementation: passed.
- `cd backend && .venv/bin/python -m ruff check src/services/execution_commit_service.py src/gateway/app.py src/gateway/health.py src/task/worker.py src/application/handlers/thread_turn_handler.py src/services/thread_events.py src/gateway/routers/thread_serializers.py src/gateway/routers/workspaces_runtime.py tests/services/test_execution_commit_service.py tests/gateway/test_health.py tests/task/test_worker.py tests/architecture/test_dataservice_boundaries.py` -> passed.
- `cd backend && .venv/bin/python -m pytest tests/services/test_execution_commit_service.py tests/gateway/test_health.py tests/task/test_worker.py tests/services/test_thread_events.py tests/gateway/routers/test_threads.py tests/gateway/routers/test_threads_router.py tests/gateway/routers/test_workspaces_serializers.py tests/application/handlers/test_thread_turn_runtime_config.py tests/architecture/test_dataservice_boundaries.py::test_reference_library_runtime_uses_dataservice_boundary tests/architecture/test_dataservice_boundaries.py::test_gateway_process_stays_off_database_lifecycle tests/architecture/test_dataservice_boundaries.py::test_worker_process_stays_off_database_lifecycle tests/architecture/test_dataservice_boundaries.py::test_runtime_type_hints_use_dataservice_contracts_not_database_models -q` -> 80 passed.
- `rg -n "from src\\.database import|from src\\.database\\.session|Depends\\(get_db\\)|get_db_session|AsyncSession|self\\.db|\\bdb=" backend/src/gateway backend/src/application backend/src/task backend/src/services backend/src/agents backend/src/tools backend/src/compute -g '*.py'` -> no matches.
- `cd backend && .venv/bin/python -m pytest tests/architecture/test_dataservice_boundaries.py tests/architecture/test_docs_current_contract.py -q` -> 43 passed.
- `cd backend && .venv/bin/python -m pytest tests/architecture/test_dataservice_boundaries.py -q` -> 41 passed.
- `cd backend && env -u ALL_PROXY -u all_proxy -u HTTP_PROXY -u http_proxy -u HTTPS_PROXY -u https_proxy .venv/bin/python -m pytest tests/ -q` -> 2041 passed.
